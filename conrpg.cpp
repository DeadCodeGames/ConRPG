...
public:
	...
	void checkLevelUp(string language){
		...
		/*else{
			cout << "\n      Zvýšila sa ti úrove? " << initiallvl << " na úrove? " << level << ". HP: " << currenthp << " -> " << hp << "\n";
	...
	void deathsequence(string language){
		...
		/*else{
			...
			if(lives!=0){
				FunctionDef.truenarrator("\tNemusíš sa ale bá?, odvážny cestovate?!", 50); FunctionDef.skip(200); FunctionDef.TNC(" Smr? je skúsenos? a skúsenos? je život.", 50); FunctionDef.skip(200); 
				if(lives==2) FunctionDef.TNC("Ostávaju ti " + to_string(lives) + " životy", 50);
				else if(lives==1) FunctionDef.TNC("Ostáva ti 1 život.", 50);
				else{
					FunctionDef.TNC("Máš nekonecno životov.", 50);
				}
				cout << "\n"; FunctionDef.skip(250);
				FunctionDef.truenarrator("\t[Stlac F na oživenie, L na zacatie novej hry.]", 50);
				while(true){
					if(_kbhit()){
				  	char key = _getch();
				  	if(key=='f'){
				...
				FunctionDef.truenarrator("\tA bol to tvoj posledný život...", 50); FunctionDef.skip(250); FunctionDef.TNC(" Je mi ?úto že sa ti toto stalo.", 50); FunctionDef.skip(250);
				cout << "\n";
	...
	int ambush(string attacker, string language) {
		...
		/*else{
			if (CreateProcess(NULL, commandLine, NULL, NULL, FALSE, CREATE_NEW_CONSOLE, NULL, NULL, &si, &pi)) {
				...
				ifstream inputFile("testsave.txt");
			if (!inputFile.is_open()) {
				cerr << "Chyba pri otvorení súboru na nacítanie údajov." << endl;
				return 1;
			}
			inputFile >> fightstatus;
			inputFile >> xpgained;
			if(fightstatus=="1"){
				FunctionDef.truenarrator("Prerušil si súboj... Automaticky umieraš.", 50); cout << "\n"; FunctionDef.skip(250); FunctionDef.truenarrator("[Stlac F pre nacítanie poslednej uloženej pozície alebo stlac L pre nacítanie novej hry.]",50);
				lives--; deathsequence(lang);
			}
			else{
				FunctionDef.truenarrator("Dokoncil si súboj.", 50); cout << "\n";
			}
		}*/
			Sleep(3000);
			xp+=xpgained;
	...
	int endingplaceholder1(string language){
		...
		/*else{
			cout << "\n\n";
			FunctionDef.truenarrator("Išiel si vlastnout cestou. Nikdy sa nedozvieš co sa stalo.", 50); FunctionDef.skip(250); cout << "\n\n";
	...
	void d1n(string language) {
		...
		/*else{
			if (a == 0) FunctionDef.truenarrator("\t  Stromy, zaujímavá vo?ba. Išiel si bližšie ku stromom, ale nic si nenašiel.", 50);
			if (a == 1) FunctionDef.truenarrator("\t  Vážne? Prišiel si znova ku stromom, a... znova si nic nenašiel.", 50);
			if (a == 2) FunctionDef.truenarrator("\t  Nieco je divné na tých stromoch. Sú ako... stromy.", 50);
			if (a > 2) {
				FunctionDef.truenarrator("\t  Išiel si k stromom ešteb raz.A opä? si nic nenašie-", 50); cout << "\n";
				FunctionDef.truenarrator("\t  C- co to je za zvuk? Vyzerá to tak... že si nieco zobudil. A je to naštvané.", 75); cout << "\n"; FunctionDef.skip(300);
				FunctionDef.truenarrator("\t  Axir, The Lord of the Forbidden Forest [BOSS] [10M HP] Nemáš šancu proti tomuto monštru zví?azi?, ke?že ?a zabije", 50); cout << "\n";
				FunctionDef.truenarrator("\t  ešte predtým, než má súboj vôbec šancu zaca?.", 50); FunctionDef.skip(400); cout << "\n"; FunctionDef.skip(400); system("cls"); cout << "\n\n\n\n";
				FunctionDef.truenarrator("\t  \033[1;31mZískal si koniec Zberac Stromov.\033[0m", 50); cout << "\n"; FunctionDef.skip(400); cout << "\n\n\n\n\n\n\n\n\n\n";
				lives--; UlozitInfo(to_string(lives), "Lives.txt"); deathsequence(lang);
			}
		}*/
	}
	...
	void introsequence(string language){
		...
		_getch();
		cout << "\n";FunctionDef.truenarrator("Zdravím ?a cestovate?!", 100); FunctionDef.skip(1000); cout << "\n";
		FunctionDef.truenarrator("...", 500); FunctionDef.skip(1000); cout << "\n";
		FunctionDef.truenarrator("Asi sa nevoláš cestovate?, že", 80); FunctionDef.skip(1500); cout << "\n";
		FunctionDef.truenarrator("Ako sa voláš?", 80); FunctionDef.skip(1000); cout << endl << "      [NAPÍŠ SVOJE MENO]" << endl;
		FunctionDef.flushInput();
		cout << ">>    "; getline(cin, input); FunctionDef.skip(1000); playerName = input; save();
		if (!playerName.empty()) {
			FunctionDef.truenarrator("Rád ?a spoznávam, "+playerName+"!", 100);
		} else {
			FunctionDef.truenarrator("Ty...", 500); FunctionDef.TNC(" ty nemáš meno?", 125); FunctionDef.skip(500); FunctionDef.TNC(" Isto máš nejaké meno. Dám ti šancu ho ešte zada?..", 80); cout << "\n"; 
			FunctionDef.flushInput();
			cout << ">>    "; getline(cin, input); FunctionDef.skip(1000); playerName = input; save();
			if (!playerName.empty()) {
			FunctionDef.truenarrator("Výborne, rád ?a spoznávam, "+playerName+"!", 80);
		} else {
			FunctionDef.truenarrator("Vážne nemáš meno?", 125); cout << "\n"; FunctionDef.skip(1000);
			FunctionDef.truenarrator("To je", 225); FunctionDef.TNC("...", 500); FunctionDef.skip(500); FunctionDef.TNC(" smutné. Je mi ?a ?úto.", 200); cout << "\n"; FunctionDef.skip(750);
			FunctionDef.truenarrator("Ale aj tak ?a nejako potrebujem vola?.", 80); cout << "\n"; FunctionDef.skip(500);
			FunctionDef.truenarrator("Môžem ?a vola? Cestovate??", 75); FunctionDef.skip(500); FunctionDef.TNC(" Myslím, že to budem fajn.", 75); playerName = "Cestovate?"; save();
		}
	} 
				FunctionDef.skip(1000); cout << "\n";
			FunctionDef.truenarrator("Na akej obtiažnosti si praješ hra? hru, drahý cestovate??", 80); FunctionDef.skip(200); cout << "\n"; FunctionDef.truenarrator("<1> ?AHKÁ", 50); FunctionDef.skip(200); cout << "\n"; FunctionDef.truenarrator("<2> NORMÁLNA", 50); FunctionDef.skip(200); cout << "\n"; FunctionDef.truenarrator("<3> ?AŽKÁ", 50); FunctionDef.skip(200); cout << "\n"; FunctionDef.truenarrator("<4> TURBO", 50);
			mode = FunctionDef.GVIclean(1,4);
			if(mode=="1") mode = "easy";
			else if(mode=="2") mode = "normal";
			else if(mode=="3") mode = "hard";
			...
			system("cls");
			if(mode=="easy"){
				cout << "\n\n\n\n\n\n"; FunctionDef.truenarrator("\t\tJednoduchá obtiažnos?...", 100); FunctionDef.skip(250);
				cout << "\n\n"; FunctionDef.truenarrator("\t\tMáš neobmedzený pocet oživení.", 50); 
				Sleep(2000); lives = -1;
			}
			else if(mode=="normal"){
				cout << "\n\n\n\n\n\n"; FunctionDef.truenarrator("\t\tNormálna obtiažnos?...", 100); FunctionDef.skip(250);
				cout << "\n\n"; FunctionDef.truenarrator("\t\tMáš k dispozícii 3 oživenia.", 50); 
				Sleep(2000); lives = 3;
			}
			else if(mode=="hard"){
				cout << "\n\n\n\n\n\n"; FunctionDef.truenarrator("\t\t?ážká obtiažnos?...", 100); FunctionDef.skip(250);
				cout << "\n\n"; FunctionDef.truenarrator("\t\tMáš k dispozícii jedno oživenie.", 50); 
				Sleep(2000); lives = 1;
			}
			else{
				cout << "\n\n\n\n\n\n"; FunctionDef.truenarrator("\t\tSpeedrun mode...", 100); FunctionDef.skip(250);
				cout << "\n\n"; FunctionDef.truenarrator("\t\tMáš jedno oživenie a hra sa neukladá. Tvoja cesta bude ma? tú správnu rýchlos?.", 50); 
				Sleep(2000); lives = 1;
			}
			UlozitInfo(to_string(lives), "Lives.txt");
			sequence=0;
	...
	void sequence1(string language){
		...
		/*else{
		if(saveload==1){
				cout << "\n\n\n"; FunctionDef.truenarrator("\tVitaj spät cestovatel. Chýbal si nám. Môžeš pokracova? "+mode+ " na svojej ceste.", 50); Sleep(2500);
				system("cls");
			}
			if(sequence==0) sequence++;
			save();
			Sleep(500);
		cout << "\n\n\n\n\n\n\t\t{[()]} Akt 1 {[()]}\n\n\n"; Sleep(500);
		FunctionDef.truenarrator("\t\tHra automaticky ukladá každý akt. Ve?a š?astia na cestách.", 50);
		Sleep(2000); system("cls");
		Sleep(2500);
		FunctionDef.MCS("Kde to som?", 80);FunctionDef.narratorclean(mc," Ako som sa sem dostal?",80); FunctionDef.skip(1000);
		cout << "\n";
		FunctionDef.MCS("Všade okolo m?a je len hustý les.", 80); FunctionDef.skip(1000);
		cout << "\n";
		FunctionDef.MCS("Ja, ", 100);FunctionDef.narratorclean(mc,"...", 200); FunctionDef.skip(500);FunctionDef.narratorclean(mc," nic si nepamätám.", 80); FunctionDef.skip(1500);
		cout << "\n";
		FunctionDef.narrator("Co sa deje?! ", 80); FunctionDef.skip(600);FunctionDef.narratorclean(vc,"Preco nemôžem ovláda? svoje telo? ", 100); FunctionDef.skip(500);FunctionDef.narratorclean(vc,"Pocka?, toto... ", 175); FunctionDef.skip(300);FunctionDef.narratorclean(vc,"Toto nie je moje telo!", 80);
		cout << "\n";
		FunctionDef.MCS("Kto to povedal? ", 40); FunctionDef.skip(750);
		cout << "\n";
		FunctionDef.MCS("Kde si? Ukáž sa!", 75); FunctionDef.skip(1000);
		cout << "\n";
		FunctionDef.narrator("Nemôžem sa ukáza?.", 80); FunctionDef.skip(1000); cout << "\n";
		...
		if(diadec2=="1"){
			...
			cout << "\n";
			FunctionDef.narrator("Dobrá otázka.", 80); FunctionDef.skip(500);FunctionDef.narratorclean(vc," Všetko co si pamätám, je moja smr?.", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc," a zrazu,", 125);FunctionDef.narratorclean(vc," som sa ocitol tu,", 100); FunctionDef.skip(500);FunctionDef.narratorclean(vc," v tvojom tele.", 100); FunctionDef.skip(1000);
			cout << endl << endl;FunctionDef.truenarrator("1) Co myslíš pod tvojou smr?ou?", 50); cout << "\n";FunctionDef.truenarrator("2) Pockaj, ty si m?tvy?",50); cout << "\n";FunctionDef.truenarrator("3) Toto je sen, že?",50); cout << "\n";FunctionDef.truenarrator("4) Co?",50); 		
			cout << "\n\n\033[33m>>    ";
			diadec3 = FunctionDef.GVIclean(1,4);
			cout << "\033[0m";
			if(diadec3=="1"){
				FunctionDef.MCSC(mc,"Co myslíš pod tvojou smr?ou?", 80); FunctionDef.skip(1000);
				cout << "\n";
				FunctionDef.narrator("Toto musí by? nocná mora.", 80); FunctionDef.skip(1000);
				cout << "\n";
				...
				cout << "\n";
				FunctionDef.narrator("Medzitým ti skúsim vysvetli? co som myslel pod \"smr?ou\" a taktiež skúsime zisti?, preco som v tvojej hlave ", 80);
				cout << "\n";
				FunctionDef.narrator("a preco si nic nepamätáš.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," Postav sa a po?me inde, skôr než nás niekto", 100);FunctionDef.skip(200);FunctionDef.narratorclean(vc," alebo horšie,", 125); FunctionDef.skip(200);FunctionDef.narratorclean(vc," nieco nájde.", 100); FunctionDef.skip(1000);
				cout << endl << endl;FunctionDef.truenarrator("1) Preco by som ?a mmal pocúva??", 50); cout << "\n";FunctionDef.truenarrator("2) Neverím ti.",50); cout << "\n";FunctionDef.truenarrator("3) Toto je nocná mora,že?",50); cout << "\n";FunctionDef.truenarrator("4) Dobre, mali by sme sa pohnú?.",50); 		
				cout << "\n\n\033[33m>>    ";
				diadec4 = FunctionDef.GVIclean(1,4);
				cout << "\033[0m";
				if(diadec4=="1"){
					FunctionDef.MCSC(mc,"Preco by som ?a mal pocúva??", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Myslíš, že máš na výber? Co iné chceš robi??", 75); FunctionDef.skip(1500); relwvoid = 2;
				}
				if(diadec4=="2"){
					FunctionDef.MCSC(mc,"Neverím ti.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Nemusíš mi veri?.", 75); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " Stací, ke? sa postavíš a pohneš tým svojim zadkom predtým než nás nieco nájde a obidvoch nás zabije.", 75); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.MCS("Fajn.", 100); FunctionDef.skip(1500); relwvoid = 2;
				}
				if(diadec4=="3"){
					FunctionDef.MCSC(mc,"Toto je nocná mora,že?", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Ale no tak, vschop sa.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," Jasné, že toto nie je nocná mora.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," Aj ke? možno aj je, ale na tom nezáleží.", 80); FunctionDef.skip(200); cout << "\n";
					FunctionDef.narrator("Umiera? vo sne tiež stojí za nic.", 80);  FunctionDef.skip(400);FunctionDef.narratorclean(vc," Tak sa prosím ?a postav a spolu na to prídeme.", 80); FunctionDef.skip(1000);
					FunctionDef.MCS("Dobre, vyhoviem ti.", 80); FunctionDef.skip(1500); relwvoid = 3;
				}
				if(diadec4=="4"){
					FunctionDef.MCSC(mc,"Oukej, mali by sme sa pohnú?.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Výborne, po?me tadia?to.", 80);  FunctionDef.skip(500);FunctionDef.narratorclean(vc," Vyzerá to tam s?ubne.", 75); FunctionDef.skip(1500); relwvoid = 4;
				}
			}
			else if(diadec3=="2"){
				FunctionDef.MCSC(mc,"Pocka?, ty si m?tvy?", 70); FunctionDef.skip(1000);
				cout << "\n";
				FunctionDef.narrator("Potvrdzujem. Teda aspo? to tak vyzerá.", 80); FunctionDef.skip(1000);
				cout << "\n";
				FunctionDef.narrator("No,", 125); FunctionDef.skip(400);FunctionDef.narratorclean(vc," nemáme zrovna cas na zbytocné premýš?ania,",80); FunctionDef.skip(300);FunctionDef.narratorclean(vc," postav sa a poobzeraj sa.", 80 ); FunctionDef.skip(250);
				cout << "\n";
				FunctionDef.narrator("Medzitým, ti skúsim vysvetli? co som myslel pod \"smr?ou\" a taktiež skúsime zisti?, preco som v tvojej hlave", 80);
				cout << "\n";
				FunctionDef.narrator("a preco si nic nepamätáš.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," Postav sa skôr než nás niekto", 100);FunctionDef.skip(200);FunctionDef.narratorclean(vc," alebo horšie,", 125); FunctionDef.skip(200);FunctionDef.narratorclean(vc," nieco nájde.", 100); FunctionDef.skip(1000);
				cout << endl << endl;FunctionDef.truenarrator("1) Preco by som ?a mal pocúva??", 50); cout << "\n";FunctionDef.truenarrator("2) Neverím ti.",50); cout << "\n";FunctionDef.truenarrator("3) Toto je nocná mora, že?",50); cout << "\n";FunctionDef.truenarrator("4) Dobre, mali by sme sa pohnú?.",50); 		
				cout << "\n\n\033[33m>>    ";
				diadec4 = FunctionDef.GVIclean(1,4);
				cout << "\033[0m";
				if(diadec4=="1"){
				...
				if(diadec4=="2"){
					FunctionDef.MCSC(mc,"Neverím ti.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Nemusíš mi veri?.", 75); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " Stací, ke? sa pohneš s tým svojím zadkom predtým, než nás nieco zabije.", 75); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.MCS("Fajn.", 100); FunctionDef.skip(1500); relwvoid = 2;
				}
				if(diadec4=="3"){
					FunctionDef.MCSC(mc,"Toto je nocná mora, že?", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Ale no tak, vzchop sa.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," Jasné, že toto nie je nocná mora.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," Aj ke?, možno aj je, ale na tom nezáleží.", 80); FunctionDef.skip(200); cout << "\n";
					FunctionDef.narrator("Umieranie v sne tiež stojí za nic.", 80);  FunctionDef.skip(400);FunctionDef.narratorclean(vc," Tak sa prosím ?a postav a spolu na to prídeme.", 80); FunctionDef.skip(1000);
					FunctionDef.MCS("Dobre, vyhoviem ti.", 80); FunctionDef.skip(1500); relwvoid = 3;
				}
				if(diadec4=="4"){
					FunctionDef.MCSC(mc,"Dobre, mali by sme sa pohnú?.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Výborne, po?me tadia?to.", 80);  FunctionDef.skip(500);FunctionDef.narratorclean(vc," Vyzerá to tam s?ubne.", 75); FunctionDef.skip(1500); relwvoid = 4;
				}
			}
				...
				cout << "\n";
				FunctionDef.narrator("Chápem, že je ?ažké pochopi? takúto situáciu,", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," ale každopádne,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(mc," ci už sen alebo realita,", 100); FunctionDef.skip(250);
				cout << "\n";
				FunctionDef.narrator("musíme sa pohnú?. Takže by som ti navrhoval vsa? skôr než nás niekto", 125); FunctionDef.skip(400);FunctionDef.narratorclean(vc," alebo horšie,",80); FunctionDef.skip(300);FunctionDef.narratorclean(vc," nieco,", 80 ); FunctionDef.skip(250);FunctionDef.narratorclean(vc," nájde.", 80 );
				cout << "\n";
				FunctionDef.narrator("Medzitým, ti skúsim vysvetli?, co som myslel pod \"smr?ou\" a taktiež skúsime zisti?, preco som v tvojej hlave", 80);
				cout << "\n";
				FunctionDef.narrator("a preco si nic nepamätáš.", 80);
				...
				if(diadec4=="3"){
					...
					cout << "\n";
					FunctionDef.narrator("Vysvetlím ti všetko, co viem, akonáhle sa odtia?to dostaneme.", 80); FunctionDef.skip(1500); relwvoid = 3;
				}
				if(diadec4=="4"){
					FunctionDef.MCSC(mc,"Dobre, mali by sme sa pohnú?.", 80); FunctionDef.skip(1000);
					cout << "\n";
				...
				cout << "\n";
				FunctionDef.narrator("Chápem, že to teraz asi nedokážeš pochopi?,", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," ale každopádne,", 100); FunctionDef.skip(250); 
				cout << "\n";
				FunctionDef.narrator("musíme sa pohnú?. Takže by som ti radil vsta? a odís? odtia?to skôr než nás niekto", 125); FunctionDef.skip(400);FunctionDef.narratorclean(vc," alebo horšie,",80); FunctionDef.skip(300);FunctionDef.narratorclean(vc," nieco,", 80 ); FunctionDef.skip(250);FunctionDef.narratorclean(vc," nájde.", 80 );
				cout << "\n";
				FunctionDef.narrator("Medzitým, ti skúsim vysvetli?, co som myslel pod \"death\" a taktiež skúsime prís? na to, preco som v tvojej hlave", 80);
				cout << "\n";
				FunctionDef.narrator("a preco si nic nepamätáš.", 80);
				cout << endl << endl;FunctionDef.truenarrator("1) Preco by som ?a mal pocúva??", 50); cout << "\n";FunctionDef.truenarrator("2) Neverím ti.",50); cout << "\n";FunctionDef.truenarrator("3) Toto je nocná mora, že?",50); cout << "\n";FunctionDef.truenarrator("4) Dobre, mali by sme sa pohnú?.",50); 		
				cout << "\n\n\033[33m>>    ";
				...
				if(diadec4=="2"){
					FunctionDef.MCSC(mc,"Neverím ti.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Nemusíš mi veri?.", 75); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " Stací, ke? pohneš s tým svojím zadkom skôr, než nás nieco zabije.", 75); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.MCS("Fajn.", 100); FunctionDef.skip(1500); relwvoid = 2;
				}
				if(diadec4=="3"){
					FunctionDef.MCSC(mc,"Toto je nocná mora, že?", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Ale no tak, vzchop sa.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," Jasné, že toto nie je nocná mora.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," Aj ke? možno je, ale na tom nezáleží.", 80); FunctionDef.skip(200); cout << "\n";
					FunctionDef.narrator("Umiera? v sne tiež stojí za dve veci.", 80);  FunctionDef.skip(400);FunctionDef.narratorclean(vc," Tak sa už prosím ?a postav a spolu to nejako rozlúskneme.", 80); FunctionDef.skip(1000); cout << "\n";
					FunctionDef.MCS("Dobre, vyhoviem ti.", 80); FunctionDef.skip(1500); relwvoid = 3;
				}
				if(diadec4=="4"){
					FunctionDef.MCSC(mc,"Dobre, mali by sme sa pohnú?.", 80); FunctionDef.skip(1000);
			...
			cout << "\n";
			FunctionDef.narrator("Netuším.", 80); FunctionDef.skip(500);FunctionDef.narratorclean(vc," Ale co viem,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc," je to,", 125);FunctionDef.narratorclean(vc," že by sme mali odtia?to vypadnú?,", 100); FunctionDef.skip(500); cout << "\n";FunctionDef.narrator("nechceme, aby nás tu coko?vek našlo, že?", 80); FunctionDef.skip(1000);FunctionDef.narratorclean(vc," Odpovede ti dám po ceste,", 80); FunctionDef.skip(250); cout << "\n";FunctionDef.narrator("proste sa postav a daj do pohybu,", 80); FunctionDef.skip(200);FunctionDef.narratorclean(vc, " prosím.", 100); FunctionDef.skip(1000);
			cout << endl << endl;FunctionDef.truenarrator("1) Preco by som ?a mal pocúva??", 50); cout << "\n";FunctionDef.truenarrator("2) Neverím ti.",50); cout << "\n";FunctionDef.truenarrator("3) Toto je nocná mora, že?",50); cout << "\n";FunctionDef.truenarrator("4) Dobre, mali by sme sa pohnú?.",50); 		
			cout << "\n\n\033[33m>>    ";
			diadec3 = FunctionDef.GVIclean(1,4);
			cout << "\033[0m";
			...
			if(diadec3=="3"){
				...
				cout << "\n";
				FunctionDef.narrator("Ale no tak, vzchop sa.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," Jasné, že toto nie je nocná mora.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," Aj ke?, možno je, ale na tom nezáleží.", 80); FunctionDef.skip(200); cout << "\n";
				FunctionDef.narrator("Umiera? v sne tiež za moc nestojí.", 80);  FunctionDef.skip(400);FunctionDef.narratorclean(vc," Tak sa už prosím ?a postav a spolu to nejako rozlúskneme.", 80); FunctionDef.skip(1000); cout << "\n";
				FunctionDef.MCS("Dobre, vyhoviem ti.", 80); FunctionDef.skip(1500); relwvoid = 3;
			}
			if(diadec3=="4"){
				FunctionDef.MCSC(mc,"Dobre, mali by sme sa pohnú?.", 80); FunctionDef.skip(1000);
		...
		if(diadec2=="3"){
			FunctionDef.MCSC(mc, "Ale kde sú moje spomienky?", 80); FunctionDef.skip(400);FunctionDef.narratorclean(mc," Len tak si sem prišiel, a všetky sú prec.", 80); FunctionDef.skip(400); FunctionDef.narratorclean(mc, " Co si snimi urobil?", 80); FunctionDef.skip(1000);
			cout << "\n";
			FunctionDef.narrator("Ja som tvojim spomienkam nic neurobil.", 80); FunctionDef.skip(500);FunctionDef.narratorclean(vc," Nic o tom neviem,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc," rovnako ako ty.", 125); FunctionDef.skip(400);FunctionDef.narratorclean(vc," myslíš, že tu chcem by??", 100); FunctionDef.skip(500); cout << "\n";FunctionDef.narrator("Umrel som ani neviem ako a zrazu som v cudzom tele,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " v tvojom tele,", 100);FunctionDef.narratorclean(vc, " ktoré nemôžem kontrolova?.", 100); FunctionDef.skip(250); cout << "\n";FunctionDef.narrator("Ale,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc," sme tam, kde sme,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc," tak pohni s tým svojím zadkom a postav sa.", 80); FunctionDef.skip(1000);
			cout << endl << endl;FunctionDef.truenarrator("1) Preco by som ?a mal pocúva??", 50); cout << "\n";FunctionDef.truenarrator("2) Neverím ti.",50); cout << "\n";FunctionDef.truenarrator("3) Toto musí by? nocná mora.",50); cout << "\n";FunctionDef.truenarrator("4) Dobre, mali by sme sa pohnú?.",50); 	
			cout << "\n\n\033[33m>>    ";
			diadec3 = FunctionDef.GVIclean(1,4);
			cout << "\033[0m";
			if(diadec3=="1"){
			...
			if(diadec3=="3"){
				FunctionDef.MCSC(mc,"Toto musí by? nocná mora!...", 80);FunctionDef.narratorclean(mc, "ARGHHHH!", 80); FunctionDef.skip(1000);
				cout << "\n";
				FunctionDef.narrator("Nemáme na výber,", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," musíme spolupracova?,", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," jedine, že by si chcel umrie?.", 80); FunctionDef.skip(1000); 
				cout << endl << endl;FunctionDef.truenarrator("1) Dobre, tak po?me.", 50); cout << "\n";FunctionDef.truenarrator("2) Moment, preco by som mal umrie??",50); cout << "\n";FunctionDef.truenarrator("3) Nie! Chcem ís? vlastnou cestou.",50); cout << "\n";FunctionDef.truenarrator("4) Nepohnem sa odtia?to, kým mi nepovieš, kto si.",50); 	
				cout << "\n\n\033[33m>>    ";
				diadec4 = FunctionDef.GVIclean(1,4); 
				cout << "\033[0m";
				if(diadec4=="1"){
				...
				if(diadec4=="2"){
					...
					cout << "\n";
					FunctionDef.narrator("Pretože budeš na tomto svete sám,", 75); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " bez spomienok,", 75); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " ukradtnutý každému,", 75); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " a ?ahký cie? pre nepriate?ov.", 75); FunctionDef.skip(250); cout << "\n";FunctionDef.narrator("V tomto lese nie sme sami.", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " A ja som ten, ktorý ti pomôže sa odtia?to bezpecne dosta?.", 75); FunctionDef.skip(1000); 
					cout << "\n";
					FunctionDef.MCS("Preco sa mi toto deje?", 75); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Viem, že si vystrašený a frustrovaný.", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc," Ale ver mi,", 100); FunctionDef.skip(400);FunctionDef.narratorclean(vc," naša jediná šanca, ako preži?, je postavi? sa pohnú? sa odtia?to.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.MCS("Vyzerá to tak, že nemám na výber...", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc," Dobre,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(mc," budem spolupracova?.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Super. Tak sa vydajme na cestu.", 80); FunctionDef.skip(1500); relwvoid = 3;
				}
				if(diadec4=="3"){
					FunctionDef.MCSC(mc,"Nie! Chcem ís? vlastnou cestou.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Vieš co, fajn, cho? si svojou cestou, uvidíme, kam nás to zavedie.", 75); FunctionDef.skip(1500);
					// endingplaceholder1();
				}
				if(diadec4=="4"){
					FunctionDef.MCSC(mc,"Nepohnem sa odtia?to, kým mi nepovieš, kto si.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Dobre, dobre.", 80);  FunctionDef.skip(500);FunctionDef.narratorclean(vc," Všetko ti poviem...", 75); FunctionDef.skip(250);FunctionDef.narratorclean(vc," Volám sa Void.", 75); FunctionDef.skip(250);FunctionDef.narratorclean(vc," Bol som generálom frakcie", 75); cout << "\n";FunctionDef.narrator("ktorej meno bolo Terragnis.", 75); FunctionDef.skip(250);FunctionDef.narratorclean(vc," Boli sme vo vojne s inou frakciou, volali sa Slofridi.", 75); FunctionDef.skip(250);FunctionDef.narratorclean(vc," Bol som uprostred boja,", 80); FunctionDef.skip(250); cout << "\n";FunctionDef.narrator("ke? ma zasiahol šíp.", 75); FunctionDef.skip(250);FunctionDef.narratorclean(vc," Ale,", 100); FunctionDef.skip(400);FunctionDef.narratorclean(vc," nejakým zázrakom,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc," som neumrel.", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc," Zrazu,", 100); FunctionDef.skip(250); cout << "\n";FunctionDef.narrator(" som videl ostré svetlo, po ktorom nasledovala tma.", 75); FunctionDef.skip(250);FunctionDef.narratorclean(vc," Ke? som prišiel k vedomiu, našiel som sa v cudzom tele...", 75); FunctionDef.skip(250); cout << "\n";FunctionDef.narrator("v tvojom tele.", 75); FunctionDef.skip(400);FunctionDef.narratorclean(vc," A teraz,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc," sme tu.", 100); FunctionDef.skip(1500);
					cout << "\n";
					...
					cout << "\n";
					FunctionDef.narrator("Povedal som ti všetko, co viem.", 80);  FunctionDef.skip(500);FunctionDef.narratorclean(vc," Tiež by som rád vedel viac,", 75); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " ale nic nezistíme, pokia? tu budeme len tak sedie?.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.MCS("Máš pravdu,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc," po?me odtia?to prec.", 75); FunctionDef.skip(1500);	relwvoid = 3;
					earlyexplanation = true;	
				}
			}
		}
			...
			cout << "\n";
			FunctionDef.narrator("Nie som klamár.", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " Iba vtedy, ke? je to fakt nevyhnutné.", 80); FunctionDef.skip(250); cout << "\n";FunctionDef.narrator("Ale zato ty by si sa mal pohnú? s tým zadkom, inak kvôli tebe obidvaja umrieme.", 80); FunctionDef.skip(1000);
			cout << endl << endl;FunctionDef.truenarrator("1) Fajn, pohnime sa.", 50); cout << "\n";FunctionDef.truenarrator("2) Vyhoviem ti, ke? mi podáš lepšie vysvetlenie.",50); cout << "\n";FunctionDef.truenarrator("3) Nechápem, ako je to možné?",50); cout << "\n";FunctionDef.truenarrator("4) Tak toto ti neverím. Iba sa ma snažíš vyláka? von.",50);
			cout << "\n\n\033[33m>>    ";
			diadec3 = FunctionDef.GVIclean(1,4);
			cout << "\033[0m";
			if(diadec3=="1"){
				...
				cout << "\n";
				FunctionDef.narrator("Super.", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, "Vydajme sa tadia?to,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " vyzerá to tam s?ubne.", 80); FunctionDef.skip(1500); relwvoid = 3;
			}
			else if(diadec3=="2"){
				FunctionDef.MCSC(mc, "Vyhoviem ti, ke? mi podáš lepšie vysvetlenie.", 80); FunctionDef.skip(1000);
				cout << "\n";
				FunctionDef.narrator("Povedal som ti,", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " fakt neviem.", 80);FunctionDef.narratorclean(vc, " Jediné co viem je, že som uväznený v tvojej hlave a že nás dostaneš ", 80); cout << "\n";FunctionDef.narrator("do hrobu pokia? sa odtia?to nepohneš,",80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " tak ?a prosím,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " pohni sa,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " a poviem ti viac po ceste.", 80); FunctionDef.skip(1000); 
				cout << endl << endl;FunctionDef.truenarrator("1) Preco by som ?a mal pocúva??", 50); cout << "\n";FunctionDef.truenarrator("2) Nie, stále ti neverím. Odmietam tomuto všetkému veri?.", 50); cout << "\n";FunctionDef.truenarrator("3) Fajn, tak sa teda dám do pohybu", 50);
				cout << "\n\n\033[33m>>    ";
				diadec4 = FunctionDef.GVIclean(1,3);
				cout << "\033[0m";
				if(diadec4=="1"){
					FunctionDef.MCSC(mc, "Preco by som ?a mal pocúva??", 80); FunctionDef.skip(1000);
					FunctionDef.narrator("Nemáš na výber.", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " Aj ke?, možno máš,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " rob si co chceš,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " chceš tak ve?mi umrie??", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Prosím, v k?ude si cho?.", 80); FunctionDef.skip(1000);
					cout << endl << endl;FunctionDef.truenarrator("1) Pôjdem svojou vlastnou cestou.", 50); cout << "\n";FunctionDef.truenarrator("2) Dobre, pockaj, ospravedl?ujem sa. Urobím, co chceš.", 50);
					cout << "\n\n\033[33m>>    ";
					diadec5 = FunctionDef.GVIclean(1,4);
					cout << "\033[0m";
					if(diadec5=="1"){
						...
						cout << "\n";
						FunctionDef.narrator("Vieš co, fajn, cho? kam chceš, uvidíme, kam sa dostaneš.", 80);
						// endingplaceholder1();
					}
					else{
						FunctionDef.MCSC(mc, "Dobre,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " prepác.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(mc, " Urobím, co chceš.", 80); FunctionDef.skip(1000);
						cout << "\n";
						FunctionDef.narrator("Tak predsa len.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Nuž,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " nemáme casu nazvyš,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " povsta? a vydaj sa tamtou cestou.", 80); FunctionDef.skip(1500); relwvoid = 3;
					}
				}
				else if(diadec4=="2"){
					FunctionDef.MCSC(mc, "Nie,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " stále ti neverím.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(mc, " Odmietam tomuto všetkému veri?.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Aha,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " takže tu chceš umrie??", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Prosím,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " cho? kam chceš.", 80); FunctionDef.skip(1000);
					cout << endl << endl;FunctionDef.truenarrator("1) Pôjdem tam, kde chcem.", 50); cout << "\n";FunctionDef.truenarrator("2) Vieš co, fajn, pocúvnem ?a.", 50);
					cout << "\n\n\033[33m>>    ";
					diadec5 = FunctionDef.GVIclean(1,2);
					cout << "\033[0m";
					if(diadec5=="1"){
						...
						cout << "\n";
						FunctionDef.narrator("Vieš co, ako chceš, cho? svojou cestou. Aspo? uvidíme, kam to vedie.", 80);
						// endingplaceholder1();
					}
					else{
						FunctionDef.MCSC(mc, "Vieš co,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " fajn,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " pocúvnem ?a.", 80); FunctionDef.skip(1000);
						cout << "\n";
						FunctionDef.narrator("Konecne...", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Môžeme sa pohnú?.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Postav sa,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " a po?me touto cestou.", 80); FunctionDef.skip(1500); relwvoid = 1;
					}
				}
				else{
					FunctionDef.MCSC(mc, "Fajn,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " dám sa do pohybu.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Dobre,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " v poriadku,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " mali by sme ís? tadia?to.", 80); FunctionDef.skip(1500); relwvoid = 3;
				}
			}
			else if(diadec3=="3"){
				FunctionDef.MCSC(mc, "Nerozumiem, ako je to možné?", 80); FunctionDef.skip(1000);
				cout << "\n";
				FunctionDef.narrator("Tiež tomu nerozumiem.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Môžeme to spolu zisti?,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " len sa už,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " prosím ?a,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " pohni.", 100); FunctionDef.skip(1000); 
				cout << endl << endl;FunctionDef.truenarrator("1) Jasné, prepác, dám sa do pohybu.", 50); cout << "\n";FunctionDef.truenarrator("2) Co ak nie?", 50);
				cout << "\n\n\033[33m>>    ";
				diadec4 = FunctionDef.GVIclean(1,2);
				cout << "\033[0m";
				if(diadec4=="1"){
					FunctionDef.MCSC(mc, "Jasné,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " prepác,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " dám sa do pohybu.", 90); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("V pohode", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " po?me tadia?to.", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " Zdá sa mi, že tam nieco vidím.", 80); FunctionDef.skip(1000); relwvoid = 3;
				}
				else{
					FunctionDef.MCSC(mc, "Co ak,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " nie?", 100); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Fakt si chceš zo m?a urobi? nepriate?a?", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Pamätaj,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " som v tvojej hlave,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " a kedyko?vek môžem pre teba z tohto urobi? ", 80); cout << "\n";FunctionDef.narrator("peklo na Zemi.", 80); FunctionDef.skip(1000);
					cout << endl << endl;FunctionDef.truenarrator("1) Áno. A chcem ís? svojou cestou.", 50); cout << "\n";FunctionDef.truenarrator("2) Zmenil som názor, prepác, už ?a budem pocúva?.", 50); 
					cout << "\n\n\033[33m>>    ";
					diadec5 = FunctionDef.GVIclean(1,2);
					cout << "\033[0m";
					if(diadec5=="1"){
						FunctionDef.MCSC(mc, "Áno. A chcem ís? svojou cestou.", 80); FunctionDef.skip(1000);
						cout << "\n";
						FunctionDef.narrator("Vieš co, rob si ako chceš, uvidíme, kam sa dostaneme.", 80); FunctionDef.skip(1500);
						// endingplaceholder1();
					}
					else{
						FunctionDef.MCSC(mc, "Zmenil som názor,",80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " prepác,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " už ?a budem pocúva?.", 80); FunctionDef.skip(1000);
						cout << "\n";
						FunctionDef.narrator("Tak nakoniec predsa len,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " fajn,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc, "po?me tadia?to,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " Skúsim ti vysvetli? všetko, co viem.", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " Pokia? chceme spolu nažíva?", 80); cout << "\n";FunctionDef.narrator("v mieri,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " tak musíme spolupracova?.", 80); FunctionDef.skip(1500); relwvoid = 2;
					}
				}
			}
			else{
				FunctionDef.MCSC(mc, "Na to ti neskocím.", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " Snažíš sa ma dosta? von.", 80); FunctionDef.skip(1000);
				cout << "\n";
				FunctionDef.narrator("Vážne?", 75); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " Pocúvaj,", 80);FunctionDef.narratorclean(vc," nemáš na výber,", 80);FunctionDef.narratorclean(vc," pohni sa,",100);FunctionDef.narratorclean(vc," nemáš co strati?.", 80); FunctionDef.skip(1000); 
				cout << endl << endl;FunctionDef.truenarrator("1) Presta? na m?a hovori?, chcem ís? vlastnou cestou.", 50); cout << "\n";FunctionDef.truenarrator("2) Fajn.", 50); cout << "\n";FunctionDef.truenarrator("3) Prepác, chcel som sa iba uisti?. Môžeme ís?.", 50); 
				cout << "\n\n\033[33m>>    ";
				diadec4 = FunctionDef.GVIclean(1,3);
				cout << "\033[0m";
				if(diadec4=="1"){
					FunctionDef.MCSC(mc, "Presta? na m?a hovori?,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " chcem ís? vlastnou cestou.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Vieš co, fajn, cho?, kadia? chceš, uvidíme, co z tohto vznikne.", 80); FunctionDef.skip(1500);
					// endingplaceholder1();
				}
				else if(diadec4=="2"){
					FunctionDef.MCSC(mc, "Fajn.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Tvoj postoj je otrasný,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " mohol by si prejavi? aspo? trochu úcty tomu hlasu, co máš v hlave.", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, "Poobzeraj sa trochu,", 80); FunctionDef.skip(250); cout << "\n";FunctionDef.narrator("vyzerá to, že tamtým smerom nieco je.", 80); FunctionDef.skip(1500); relwvoid = 1;
				}
				else{
					FunctionDef.MCSC(mc, "Prepác,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " chcel som sa len uisti?.", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " Môžeme sa da? do pohybu.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("To je v pohode,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " Chápem ?a.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " tak sa teda vydajme na cestu.", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " Vyzerá, že táto cesta niekam vedie.", 80); FunctionDef.skip(1500); relwvoid = 3;
				}
			}
		}
		cout << "\n";
		...
		while(true){
			...
			if(decision1=="1"){
				...
				FunctionDef.MCSC(mc, "Musím vidie? tie stromy.", 80);
				cout << endl << endl;
				d1n(lang);
				FunctionDef.skip(500);
				cout << endl << endl;
				...
				FunctionDef.MCSC(mc, "Chcel by som vedie?, kam vedie táto cesta.", 80);
				cout << endl << endl;
				FunctionDef.truenarrator("Okej, rozhodol si sa zisti?, aký je cie? tej cesty. Si si istý, že nechceš naskôr skontrolova?\n      to opustené miesto medzi stromami, kde môžu leža? užitocné veci?", 50);
				cout << endl << endl;
				FunctionDef.skip(250);
				FunctionDef.truenarrator("1) Pokracova? v pôvodnom rozhodnutí a ís? preskúma? cestu\n      2) Upusti? od pôvodného rozhodnutia a ís? preskúma? opustené miesto medzi stromami", 50);
				cout << endl << endl;
				FunctionDef.flushInput();
				cout << "\033[33m>>    ";
				...
				if(decision1sub1=="1"){
					...
					cout << endl << endl;
					FunctionDef.truenarrator("Nuž dobre, rozhodol si sa vyda? na dobrodrúžnu cest- hups, tak nic, umrel si. Dôvod: NEZNÁMY", 50);
					FunctionDef.skip(400);
					lives--; deathsequence(lang);
				}
				else if(decision1sub1=="2"){
					FunctionDef.MCSC(mc, "Myslím, že by sme mali najskôr skontrolova? to opustené miesto.", 80);
					cout << endl << endl;
					FunctionDef.truenarrator("Nakoniec si sa rozhodol skontrolova? to divné miesto s opustenými vecami na zemi. A pred sebou máš:",50); cout << "\n\n";FunctionDef.truenarrator("- Caj",50); cout << "\n";FunctionDef.truenarrator("- Železný mec",50); cout << "\n";FunctionDef.truenarrator("- ?ahko-znicite?né zlaté brnenie",50); cout << "\n";FunctionDef.truenarrator("- Štít",50); cout << "\n";FunctionDef.truenarrator("- Elixír lásky",50); cout << "\n";FunctionDef.truenarrator("- Palicu",50); cout << "\n";FunctionDef.truenarrator("- Obväzy x3",50); cout << "\n";FunctionDef.truenarrator("- Malý batoh", 50);
					break;
				}
			}
			else if(decision1=="5"){
				...
				cout << endl << endl;
				FunctionDef.truenarrator("Skvelá vo?ba, teda minimálne lepšia ako \"stromy\", priblížil si sa k opusteným veciam a vidíš nasledovné:",50); cout << "\n\n";FunctionDef.truenarrator("- Caj",50); cout << "\n";FunctionDef.truenarrator("- Železný mec",50); cout << "\n";FunctionDef.truenarrator("- ?ahko-znicite?né zlaté brnenie",50); cout << "\n";FunctionDef.truenarrator("- Štít",50); cout << "\n";FunctionDef.truenarrator("- Elixír lásky",50); cout << "\n";FunctionDef.truenarrator("- Palicu",50); cout << "\n";FunctionDef.truenarrator("- Obväzy x3",50); cout << "\n";FunctionDef.truenarrator("- Malý batoh", 50); break;
			}
		}
		...
		if(b0=="1"){
			FunctionDef.narrator("Nielen, že si úplný kretén,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc," ale ešte aj úplne hlúpy.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Preco by si si vybral stromy 3-krát po sebe?", 80); FunctionDef.skip(400); cout << "\n";FunctionDef.narrator("Vieš co,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " Radšej to ani nechcem pocu?,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " presta? stráca? náš cas, zober veci and vypadni odtia?to.", 80); FunctionDef.skip(1000);
			cout << endl << endl;FunctionDef.truenarrator("1) Prepác, fakt som chcel tie stromy.", 50); cout << "\n";FunctionDef.truenarrator("2) Ja som kretén?! Ty si kretén!", 50); cout << endl << endl; 
			cout << "\033[33m>>    ";
			diadec1 = FunctionDef.GVIclean(1,2);
			cout << "\033[0m";
			if(diadec1=="1"){
				FunctionDef.MCSC(mc, "Prepác, fakt som chcel tie stromy.", 80); FunctionDef.skip(500);
				cout << "\n";
				FunctionDef.narrator("Ulož to,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " zober veci a CHO?!", 80); FunctionDef.skip(1000); 
				cout << endl << endl;FunctionDef.truenarrator("Zobral si veci a odteraz ho môžeš nájs? vo svojom inventári, ktorým je batoh.", 50); FunctionDef.skip(500); cout << "\n";
			}
			else{
				...
				cout << "\n";
				FunctionDef.narrator("Ja?!", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Vieš co,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " cho? do certa,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " povedal som ti, co sa stalo a preco som tu.", 80); FunctionDef.skip(400); cout << "\n";FunctionDef.narrator("Tak láskavo presta?, zober veci a padaj prec odtia?to skôr, než umrieme.", 80); FunctionDef.skip(1000);
				cout << endl << endl;FunctionDef.truenarrator("Zobral si veci a odteraz ho môžeš nájs? vo svojom inventári, ktorým je batoh.", 50); FunctionDef.skip(500); cout << endl << endl;
			}
			FunctionDef.narrator("Fakt sa snažím by? milý,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " ale ty ma proste nenecháš.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Co navrhuješ teraz?", 80); FunctionDef.skip(400); cout << "\n";FunctionDef.narrator("Pôjdeme touto cestou?", 80); FunctionDef.skip(1000);
			cout << endl << endl;FunctionDef.truenarrator("1) Ís? cestou, ktorú navrhol Void.", 80); cout << "\n";FunctionDef.truenarrator("2) Ís? opacným smerom.", 50); cout << endl << endl;
			cout << "\033[33m>>    ";
			b1pathdec = FunctionDef.GVIclean(1,2);
			cout << "\033[0m";
			if(b1pathdec=="1"){
				...
				cout << "\n";
				FunctionDef.narrator("Aspo? že pocúvaš.", 80); FunctionDef.skip(1000); relwvoid = 2;
			}
			else{
				FunctionDef.MCSC(mc, "Vieš co?", 80); FunctionDef.skip(400);FunctionDef.narratorclean(mc, "nechce sa mi ?a pocúva?.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(mc, "Radšej by som šiel tdia?to,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " nemáme co strati?.", 80); 
				cout << "\n";
				FunctionDef.narrator("Vieš co,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " fajn,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " cho? do ktorejko?vek riti chceš.", 80); FunctionDef.skip(1000); relwvoid = 1; d="7";
			}
		}
		else if(b0=="2"){
			FunctionDef.narrator("Preco by si si vybral stromy 3-krát po sebe?", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Nie si náhodou", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, "-", 200);FunctionDef.narratorclean(vc, " Ospravedl?ujem sa.", 100); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Tvoj výber ma frustroval.", 80); FunctionDef.skip(400); cout << "\n";FunctionDef.narrator("Je úplne jasné, že tam nic nie je,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " strácame náš cas.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Mali by sme ís? ?alej,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " zobver veci a po?.", 80); FunctionDef.skip(1000);
			cout << "\n"; 
			FunctionDef.MCS("Prepác.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(mc, " Po?me tú cestu, co sme videli predtým.", 80); FunctionDef.skip(1000);
			cout << endl << endl;FunctionDef.truenarrator("Zobral si veci a odteraz ho môžeš nájs? vo svojom inventári, ktorým je batoh.", 50); FunctionDef.skip(500);
			cout << endl << endl; 
			FunctionDef.narrator("Pravda,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " tá cesta by nás mohla zavies? niekde, kde môžeme nájs? odpovede.", 80); FunctionDef.skip(1000);
		}
		else if(b0=="3"){
			FunctionDef.narrator("Zober to a po?me.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " A inak,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " co je to s tebou?", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Upusti od takýchto rozhodnutí,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " bude lepšie, ke? sa dohodneme.", 80); FunctionDef.skip(1000);
			cout << endl << endl;FunctionDef.truenarrator("Zobral si veci a odteraz ho môžeš nájs? vo svojom inventári, ktorým je batoh.", 50); FunctionDef.skip(500);
			...
			cout << "\033[0m";			 
			if(diadec1=="1"){
				FunctionDef.MCSC(mc, "Ospravedl?ujem sa za to...", 80); FunctionDef.skip(400);FunctionDef.narratorclean(mc, " Nemyslel som to tak,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " bol som iba opatrný.", 80); FunctionDef.skip(400); cout << "\n";FunctionDef.MCS("But Ale aspo? už vidím, že nie si zlý a že si naozaj v mojej hlave.", 80); FunctionDef.skip(1000);
				cout << "\n";
				FunctionDef.narrator("V poriadku,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " rozumiem.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Cením si tvoje ospravedlnenie.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Mali by sme sa da? do pohybu,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " po?me tou cestou, ktorú sme videli predtým.", 80); FunctionDef.skip(1000);
				relwvoid = 3;
			}
			else{
				FunctionDef.MCSC(mc, "...", 80); FunctionDef.skip(1000);
				cout << "\n";
				FunctionDef.narrator("Ja-", 150); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " chápem, že to musí by? z tvojho poh?adu ?ažké.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Ke? sa na to pozerám z tvojej perspektívy,", 80);FunctionDef.narratorclean(vc, " tiež by som sebe neveril.", 80); FunctionDef.skip(400); cout << "\n";FunctionDef.narrator("Chápem ?a,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " prepác.", 100); FunctionDef.skip(1000);
				cout << "\n";
				FunctionDef.MCS("?akujem,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " ale ja som ten, ktorý by sa tu mal ospravedl?ova?.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(mc, " Ale máš pravdu,", 80);FunctionDef.narratorclean(mc, " mali by sme sa da? do pohybu a o tomto sa porozprávame neskôr.", 80); FunctionDef.skip(1000); 
				relwvoid = 3;
			}
		}
		else if(b0=="4"){
			FunctionDef.narrator("Výborne.", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc," Tie veci môžu by? pre nás užitocné.", 80); FunctionDef.skip(400); FunctionDef.narratorclean(vc, " Zober ich a po?me ?alej.", 80); FunctionDef.skip(1000);
			cout << endl << endl;FunctionDef.truenarrator("Zobral si veci a odteraz ho môžeš nájs? vo svojom inventári, ktorým je batoh.", 50); FunctionDef.skip(500);
			cout << endl << endl;
			FunctionDef.MCS("Dobrý nápad,", 80); FunctionDef.skip(250); FunctionDef.narratorclean(mc, " nasledujme tú cestu, ktorú sme videli predtým,", 80); FunctionDef.skip(250); FunctionDef.narratorclean(mc, " hádam, že vedie do mesta.",80); FunctionDef.skip(400); FunctionDef.narratorclean("\033[33m", " Nemôže predsa vies? odnikia? nikam,", 80);FunctionDef.narratorclean("\033[33m", " že?", 100); FunctionDef.skip(1000);
		}
		else if(b0=="5"){
			FunctionDef.narrator("Užasné,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " mali sme riadne š?astie.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Mali by sme pokracova? v pohybe a dosta? sa odtia?to,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " hne? ako zoberieš tie veci.", 80); FunctionDef.skip(1000);
			cout << endl << endl;FunctionDef.truenarrator("Zobral si veci a odteraz ho môžeš nájs? vo svojom inventári, ktorým je batoh.", 50); FunctionDef.skip(500);
			cout << endl << endl;FunctionDef.MCS("Súhlasím,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " mali by sme sa odtia?to spakova?,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " máš pravdu,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " asi by sme mohli ís? tou cestou, ktorú sme videli predtým,", 80); FunctionDef.skip(250); cout << "\n";FunctionDef.MCS("isto niekam vedie.", 80); FunctionDef.skip(1000);			
			cout << "\n";
			...
			FunctionDef.narrator("Môžeš sa presta? tak extrémne zaujíma? o stromy, zobra? tie veci a vypadnú? odtia?to,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " trkvas?", 80); FunctionDef.skip(1000);
			cout << "\n";
			...
			FunctionDef.narrator("Vypadneme odtia?to,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " tamtou cestou.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Cho? a nic nehovor.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Tvoja rec ma zacína iritova?", 80); cout << "\n";FunctionDef.narrator("a to sme sa spoznali len 3 minúty dozadu.", 80); FunctionDef.skip(1000);			
		}
		else if(b0=="7"){
			FunctionDef.narrator("Super,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " niekam sa dostávame.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Ale preco si si 2-krát vybral stromy?", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Príde mi to dos? mätúce,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " ale co už,", 80); FunctionDef.skip(250); cout << "\n";FunctionDef.narrator("mali by sme pojracova? ?alej,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " môžeme ís? tou cestou, ktorú sme videli predtým.", 80); FunctionDef.skip(1000);
			cout << "\n";
			cout << endl << endl;FunctionDef.truenarrator("Zobral si veci a odteraz ho môžeš nájs? vo svojom inventári, ktorým je batoh.", 50); FunctionDef.skip(500);
			cout << "\n";
			...
			cout << "\n";
			FunctionDef.narrator("Mala by niekam vies?,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " predpokladám, že do dediny.", 80); FunctionDef.skip(1000);		
		}	
		else if(b0=="8"){
			FunctionDef.narrator("Výborne,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " zober ich,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " môžu by? užitocné.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " A hne? potom,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " by sme sa mali da? do pohybu,", 80); FunctionDef.skip(250); cout << "\n";FunctionDef.narrator("môžeme ís? tou cestou, ktorú sme videli predtým.", 80); FunctionDef.skip(1000);
			cout << "\n";
			FunctionDef.MCS("Dobre.", 100); FunctionDef.skip(1000);
			cout << endl << endl;FunctionDef.truenarrator("Zobral si veci a odteraz ho môžeš nájs? vo svojom inventári, ktorým je batoh.", 50); FunctionDef.skip(500);
			cout << "\n";
			FunctionDef.narrator("Super,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " môžeš sa da? do pohybu,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " skús by? nenápadný,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " nikdy nevieme, kto tu s nami môže by?.", 80); FunctionDef.skip(1000);			
		}
		else if(b0=="9"){
			FunctionDef.narrator("Preco by si išiel 2-krát k stromom?", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Absolútna strata casu.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Proste iba zober tie veci a daj sa do pohybu.", 80); FunctionDef.skip(1000);
			cout << "\n";
			FunctionDef.MCS("Ospravedl?ujem sa za to,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " ale súhlasím.", 100); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " Pohnem sa, hne? ako to zoberiem.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(mc, " Mali by sme ís? tou cestou, ktorú sme videli predtým.", 80); FunctionDef.skip(1000);
			cout << endl << endl;FunctionDef.truenarrator("Zobral si veci a odteraz ho môžeš nájs? vo svojom inventári, ktorým je batoh.", 50); FunctionDef.skip(500);
			cout << "\n";FunctionDef.narrator("Myslím, že je to naša jediná možnos?.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Musí vies? do dediny,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " na sto percent.", 80); FunctionDef.skip(1000);			 
		}
		if(b0!="1"&&relwvoid!=1){
			cout << endl << endl;FunctionDef.truenarrator("Si obk?úcený svorkou vlkov, ktorá je prepravená na teba zaútoci?. Musíš sa bráni?.", 50); cout << "\n";FunctionDef.truenarrator("Musíš sa rýchlo rozhodnú?, co urobíš: ", 50); cout << endl << endl;FunctionDef.truenarrator("1) POuži? palicu a bojova? s nimi",50); FunctionDef.skip(200); cout << "\n";FunctionDef.truenarrator("2) Použi? mec a bojova? s nimi", 50); cout << "\n";FunctionDef.truenarrator("3) Skúsi? ich odláka? palicou namiesto bojovania.", 50); cout << endl << endl;
			cout << "\033[33m>>    ";
			decision2 = FunctionDef.GVI(1,3);
			cout << "\033[0m" << endl;
			if(decision2=="1"){
				FunctionDef.truenarrator("Rozhodol si sa zobra? palicu a s ?ou sa vrhnú? do boja. Zahájujem súboj.", 50); FunctionDef.skip(200);
				ambush("Wolves", lang); 
			}
			else if(decision2=="2"){
				FunctionDef.truenarrator("Rozhodol si sa zobra? mec a s ním sa vrhnú? do boja. Zahájujem súboj.", 50); FunctionDef.skip(200);
				ambush("Wolves", lang); 
			}
			else if(decision2=="3"){
				int wolfescapechance = rand()%4;
				if(wolfescapechance==1){
					FunctionDef.truenarrator("Skúsil si vlkov odláka? palicou. Hodil  si ho a podarilo sa ti ich odláka?.", 50); // Achievement, 25% chance to work 
				}
				else{
					FunctionDef.truenarrator("Skúsil si vlkov odláka? palicou, ale vôbec nezareagovali a roztrhali ?a ka kúsky.", 50); FunctionDef.skip(400); cout << "\n";
					lives--; deathsequence(lang);
				}
			}
		}
		...
		if(c=="1"){
			FunctionDef.narrator("Zbláznil si sa?", 80); FunctionDef.skip(400); FunctionDef.narratorclean(vc, " Mohol si nás zabi?.", 80); FunctionDef.skip(400); FunctionDef.narratorclean(vc, " Máš š?astie, že to vyšlo.", 80); FunctionDef.skip(400); cout << "\n"; FunctionDef.narrator("No nic, pokracujme ?alej po ceste.", 80); FunctionDef.skip(1000);
			cout << "\n"; 
			FunctionDef.MCS("Bol to dobrý nápad,", 80); FunctionDef.skip(250); FunctionDef.narratorclean(mc, " v boji by sme mohli umrie?.", 80); FunctionDef.skip(400); FunctionDef.narratorclean(mc, " Odlákal som ich a teraz sme v bezpecí.", 80); FunctionDef.skip(400); cout << "\n"; FunctionDef.MCS("Idem teda ?alej.", 80); FunctionDef.skip(1000);
		}
		else if(c=="2"){
			FunctionDef.narrator("Wau!", 80); FunctionDef.skip(400); FunctionDef.narratorclean(vc, " To s tou palicou bol dobrý nápad,", 80); FunctionDef.skip(250); FunctionDef.narratorclean(vc, " necakal som, že to bude fungova?.", 80); FunctionDef.skip(1000);
			cout << "\n";
			FunctionDef.MCS("Tiež som si nemyslel, že to môže fungova?,", 80); FunctionDef.skip(250); FunctionDef.narratorclean(mc, " ale som rád, že to vyšlo.", 80); FunctionDef.skip(400); FunctionDef.narratorclean(mc, " Nezastavujme a po?me ?alej.", 80); FunctionDef.skip(1000);
		}
		else if(c=="3"){
			FunctionDef.narrator("Preco by si používal na vlkov palicu?", 80); FunctionDef.skip(400); FunctionDef.narratorclean(mc, " Máme š?astie, že sme stále nažive.", 80); FunctionDef.skip(1000);
			cout << "\n"; 
			FunctionDef.MCS("Bolo to ve?mi spontánne rozhodnutie.", 80); FunctionDef.skip(400); FunctionDef.narratorclean(mc, " Možno to bolo ubáhlené,", 80); FunctionDef.skip(250); FunctionDef.narratorclean(mc, " ale každopádne sme vyhrali.", 80); FunctionDef.skip(400); FunctionDef.narratorclean(mc, " Pokracujme ?alej.", 80); FunctionDef.skip(1000); 
		}
		else if(c=="4"){
			FunctionDef.narrator("Zaujímavé...?", 100); FunctionDef.skip(400); FunctionDef.narratorclean(vc, " Dobrá práca,", 80); FunctionDef.skip(250); FunctionDef.narratorclean(vc, " ocividne vieš, ako používa? palicu.", 80); FunctionDef.skip(400); FunctionDef.narratorclean(vc, " Ale preco si namiesto toho nepoužil mec?", 80); FunctionDef.skip(1000);
			cout << "\n";
			FunctionDef.MCS("Netuším.", 80); FunctionDef.skip(400); FunctionDef.narratorclean(mc, " Zobral som prvú vec, co mi prišla do rany a náhodou to bola zrovna palica,", 80); FunctionDef.skip(250); FunctionDef.narratorclean(mc, " tak som ju použil.", 80); FunctionDef.skip(400); FunctionDef.narratorclean(mc, " Ale na to nezáleží.", 80); FunctionDef.skip(400); FunctionDef.narratorclean(mc, " Vyhrali sme,", 100); FunctionDef.skip(250); FunctionDef.narratorclean(mc, " tak sa dajme to pohybu.", 80); FunctionDef.skip(1000);
		}
		else if(c=="5"){
			FunctionDef.narrator("Aspo? že vieš bojova? s mecom.", 80); FunctionDef.skip(400); FunctionDef.narratorclean(vc, " Pokracuj smerom vpred.", 80); FunctionDef.skip(1000);
			cout << "\n";
			FunctionDef.MCS("?akujem za uznanie, asi...", 80); FunctionDef.skip(400); FunctionDef.narratorclean(mc, " Zatia? si mi nebol ve?mi nápomocný,", 80); FunctionDef.skip(250); FunctionDef.narratorclean(mc, " vieš o tom?", 80); FunctionDef.skip(400); cout << "\n"; FunctionDef.MCS("Ale to asi nie je tvoja chyba.", 80); FunctionDef.skip(400); FunctionDef.narratorclean(mc, " Pokracujme ?alej.", 80); FunctionDef.skip(400);
		}
		else if(c=="6"){
			FunctionDef.narrator("Úžasné,", 80); FunctionDef.skip(250); FunctionDef.narratorclean(vc, " bol si super!", 80); FunctionDef.skip(400); FunctionDef.narratorclean(vc, " Mali by sme sa pohnú? ?alej skôr, než sem príde ?alšia svorka.", 80); FunctionDef.skip(400); cout << "\n"; FunctionDef.narrator("Som si istý, že je ich tu ve?a.", 80); FunctionDef.skip(1000);
			cout << "\n";
			FunctionDef.MCS("?akujem,", 80); FunctionDef.skip(250); FunctionDef.narratorclean(mc, " a súhlasím.", 80); FunctionDef.skip(400); FunctionDef.narratorclean(mc, " Po?me teda ?alej.", 80); FunctionDef.skip(1000);		
		}
		sequence2(language);
		}*/
	}
