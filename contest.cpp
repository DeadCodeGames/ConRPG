...
public:
	...
	int ambush(string attacker, string language) {
		...
		WideCharToMultiByte(CP_ACP, 0, wProgramName, -1, commandLine, MAX_PATH, NULL, NULL);
	  	if(language=="en"){
			if (CreateProcess(NULL, wProgramName, NULL, NULL, FALSE, CREATE_NEW_CONSOLE, NULL, NULL, &si, &pi)) {
	...
	void d1n(string language) {
		if(language=="en"){
			...
			if (a == 0) FunctionDef.truenarrator("\t  Stromy, zaujímavá voľba. Išiel si bližšie ku stromom, ale nic si nenašiel.", 50);
			...
			if (a > 2) {
				FunctionDef.truenarrator("\t  Išiel si k stromom ešteb raz.A opäť si nic nenašie-", 50); cout << "\n";
				FunctionDef.truenarrator("\t  C- co to je za zvuk? Vyzerá to tak... že si nieco zobudil. A je to naštvané.", 75); cout << "\n"; FunctionDef.skip(300);
				FunctionDef.truenarrator("\t  Axir, The Lord of the Forbidden Forest [BOSS] [10M HP] Nemáš šancu proti tomuto monštru zvíťaziť, keďže ťa zabije", 50); cout << "\n";
				FunctionDef.truenarrator("\t  ešte predtým, než má súboj vôbec šancu zacať.", 50); FunctionDef.skip(400); cout << "\n"; FunctionDef.skip(400); system("cls"); cout << "\n\n\n\n";
	...
	void savesequence(string language) {
		if(language=="en"){
			...
			FunctionDef.truenarrator("\tChceš nacítať poslednú uloženú pozíciu alebo zacať novú hru? [F pre nacítanie pozície, L pre novú hru]", 50);
		}
		while (true) {
			if (_kbhit()) {
	...
	void introsequence(string language){
		...
		cout << "                                            V I T A J    V" << endl;
	 	cout << " .----------------.    .----------------.  .----------------.  .-----------------.   .----------------. " << endl << "| .--------------. |  | .--------------. || .--------------. || .--------------. |  | .--------------. |" << endl << "| |              | |  | |     ______   | || |     ____     | || | ____  _____  | |  | |              | |" << endl << "| |              | |  | |   .' ___  |  | || |   .'    `.   | || ||_   \\|_   _| | |  | |              | |" << endl << "| |    ______    | |  | |  / .'   \\_|  | || |  /  .--.  \\  | || |  |   \\ | |   | |  | |    ______    | |" << endl << "| |   |______|   | |  | |  | |         | || |  | |    | |  | || |  | |\\ \\| |   | |  | |   |______|   | |" << endl << "| |              | |  | |  \\ `.___.'\\  | || |  \\  `--'  /  | || | _| |_\\   |_  | |  | |              | |" << endl << "| |              | |  | |   `._____.'  | || |   `.____.'   | || ||_____|\\____| | |  | |              | |" << endl << "| |              | |  | |              | || |              | || |              | |  | |              | |" << endl << "| '--------------' |  | '--------------' || '--------------' || '--------------' |  | '--------------' |" << endl << " '----------------'    '----------------'  '----------------'  '----------------'    '----------------' ";
		cout << endl << "                                           The C++ Console RPG";
		cout << endl << "                                         Stlac [cokoľvek] a zacni hru"; 
		_getch();
		cout << "\n";FunctionDef.truenarrator("Zdravím ťa cestovateľ!", 100); FunctionDef.skip(1000); cout << "\n";
		FunctionDef.truenarrator("...", 500); FunctionDef.skip(1000); cout << "\n";
		FunctionDef.truenarrator("Asi sa nevoláš cestovateľ, že", 80); FunctionDef.skip(1500); cout << "\n";
		...
		if (!playerName.empty()) {
			...
			FunctionDef.truenarrator("Ty...", 500); FunctionDef.TNC(" ty nemáš meno?", 125); FunctionDef.skip(500); FunctionDef.TNC(" Isto máš nejaké meno. Dám ti šancu ho ešte zadať..", 80); cout << "\n"; 
			FunctionDef.flushInput();
			cout << ">>    "; getline(cin, input); FunctionDef.skip(1000); playerName = input; save();
			if (!playerName.empty()) {
			...
			FunctionDef.truenarrator("To je", 225); FunctionDef.TNC("...", 500); FunctionDef.skip(500); FunctionDef.TNC(" smutné. Je mi ťa ľúto.", 200); cout << "\n"; FunctionDef.skip(750);
			FunctionDef.truenarrator("Ale aj tak ťa nejako potrebujem volať.", 80); cout << "\n"; FunctionDef.skip(500);
	...
	void sequence1(string language){
		...
		if(saveload==1){
				cout << "\n\n\n"; FunctionDef.truenarrator("\tVitaj späť cestovateľ. Chýbal si nám. Môžeš pokracovať "+mode+ " na svojej ceste.", 50); Sleep(2500);
				system("cls");
			}
			if(sequence==0) sequence++;
			save();
		...
		cout << "\n";
		FunctionDef.narrator("Co sa deje?! ", 80); FunctionDef.skip(600);FunctionDef.narratorclean(vc,"Preco nemôžem ovládať svoje telo? ", 100); FunctionDef.skip(500);FunctionDef.narratorclean(vc,"Pockať, toto... ", 175); FunctionDef.skip(300);FunctionDef.narratorclean(vc,"Toto nie je moje telo!", 80);
		cout << "\n";
		FunctionDef.MCS("Kto to povedal? ", 40); FunctionDef.skip(750);
		cout << "\n";
		FunctionDef.MCS("Kde si? Ukáž sa!", 75); FunctionDef.skip(1000);
		cout << "\n";
		FunctionDef.narrator("Nemôžem sa ukázať.", 80); FunctionDef.skip(1000); cout << "\n";
		...
		if(diadec2=="1"){
			...
			cout << "\n";
			FunctionDef.narrator("Dobrá otázka.", 80); FunctionDef.skip(500);FunctionDef.narratorclean(vc," Všetko co si pamätám, je moja smrť.", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc," a zrazu,", 125);FunctionDef.narratorclean(vc," som sa ocitol tu,", 100); FunctionDef.skip(500);FunctionDef.narratorclean(vc," v tvojom tele.", 100); FunctionDef.skip(1000);
			cout << endl << endl;FunctionDef.truenarrator("1) Co myslíš pod tvojou smrťou?", 50); cout << "\n";FunctionDef.truenarrator("2) Pockaj, ty si mŕtvy?",50); cout << "\n";FunctionDef.truenarrator("3) Toto je sen, že?",50); cout << "\n";FunctionDef.truenarrator("4) Co?",50); 		
			cout << "\n\n\033[33m>>    ";
			diadec3 = FunctionDef.GVIclean(1,4);
			cout << "\033[0m";
			if(diadec3=="1"){
				FunctionDef.MCSC(mc,"Co myslíš pod tvojou smrťou?", 80); FunctionDef.skip(1000);
				cout << "\n";
				FunctionDef.narrator("Toto musí byť nocná mora.", 80); FunctionDef.skip(1000);
				cout << "\n";
				...
				cout << "\n";
				FunctionDef.narrator("Medzitým ti skúsim vysvetliť co som myslel pod \"smrťou\" a taktiež skúsime zistiť, preco som v tvojej hlave ", 80);
				cout << "\n";
				FunctionDef.narrator("a preco si nic nepamätáš.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," Postav sa a poďme inde, skôr než nás niekto", 100);FunctionDef.skip(200);FunctionDef.narratorclean(vc," alebo horšie,", 125); FunctionDef.skip(200);FunctionDef.narratorclean(vc," nieco nájde.", 100); FunctionDef.skip(1000);
				cout << endl << endl;FunctionDef.truenarrator("1) Preco by som ťa mmal pocúvať?", 50); cout << "\n";FunctionDef.truenarrator("2) Neverím ti.",50); cout << "\n";FunctionDef.truenarrator("3) Toto je nocná mora,že?",50); cout << "\n";FunctionDef.truenarrator("4) Dobre, mali by sme sa pohnúť.",50); 		
				cout << "\n\n\033[33m>>    ";
				diadec4 = FunctionDef.GVIclean(1,4);
				cout << "\033[0m";
				if(diadec4=="1"){
					FunctionDef.MCSC(mc,"Preco by som ťa mal pocúvať?", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Myslíš, že máš na výber? Co iné chceš robiť?", 75); FunctionDef.skip(1500); relwvoid = 2;
				}
				if(diadec4=="2"){
					FunctionDef.MCSC(mc,"Neverím ti.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Nemusíš mi veriť.", 75); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " Stací, keď sa postavíš a pohneš tým svojim zadkom predtým než nás nieco nájde a obidvoch nás zabije.", 75); FunctionDef.skip(1000);
					cout << "\n";
				...
				if(diadec4=="3"){
					...
					cout << "\n";
					FunctionDef.narrator("Ale no tak, vschop sa.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," Jasné, že toto nie je nocná mora.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," Aj keď možno aj je, ale na tom nezáleží.", 80); FunctionDef.skip(200); cout << "\n";
					FunctionDef.narrator("Umierať vo sne tiež stojí za nic.", 80);  FunctionDef.skip(400);FunctionDef.narratorclean(vc," Tak sa prosím ťa postav a spolu na to prídeme.", 80); FunctionDef.skip(1000);
				...
				FunctionDef.MCSC(mc,"Pockať, ty si mŕtvy?", 70); FunctionDef.skip(1000);
				cout << "\n";
				FunctionDef.narrator("Potvrdzujem. Teda aspoň to tak vyzerá.", 80); FunctionDef.skip(1000);
				cout << "\n";
				FunctionDef.narrator("No,", 125); FunctionDef.skip(400);FunctionDef.narratorclean(vc," nemáme zrovna cas na zbytocné premýšľania,",80); FunctionDef.skip(300);FunctionDef.narratorclean(vc," postav sa a poobzeraj sa.", 80 ); FunctionDef.skip(250);
				cout << "\n";
				FunctionDef.narrator("Medzitým, ti skúsim vysvetliť co som myslel pod \"smrťou\" a taktiež skúsime zistiť, preco som v tvojej hlave", 80);
				cout << "\n";
				...
				cout << endl << endl;FunctionDef.truenarrator("1) Preco by som ťa mal pocúvať?", 50); cout << "\n";FunctionDef.truenarrator("2) Neverím ti.",50); cout << "\n";FunctionDef.truenarrator("3) Toto je nocná mora, že?",50); cout << "\n";FunctionDef.truenarrator("4) Dobre, mali by sme sa pohnúť.",50); 		
				cout << "\n\n\033[33m>>    ";
				diadec4 = FunctionDef.GVIclean(1,4);
				cout << "\033[0m";
				if(diadec4=="1"){
				...
				if(diadec4=="2"){
					FunctionDef.MCSC(mc,"Neverím ti.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Nemusíš mi veriť.", 75); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " Stací, keď sa pohneš s tým svojím zadkom predtým, než nás nieco zabije.", 75); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.MCS("Fajn.", 100); FunctionDef.skip(1500); relwvoid = 2;
				}
				if(diadec4=="3"){
					FunctionDef.MCSC(mc,"Toto je nocná mora, že?", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Ale no tak, vzchop sa.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," Jasné, že toto nie je nocná mora.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," Aj keď, možno aj je, ale na tom nezáleží.", 80); FunctionDef.skip(200); cout << "\n";
					FunctionDef.narrator("Umieranie v sne tiež stojí za nic.", 80);  FunctionDef.skip(400);FunctionDef.narratorclean(vc," Tak sa prosím ťa postav a spolu na to prídeme.", 80); FunctionDef.skip(1000);
					FunctionDef.MCS("Dobre, vyhoviem ti.", 80); FunctionDef.skip(1500); relwvoid = 3;
				}
				if(diadec4=="4"){
				...
				cout << "\n";
				FunctionDef.narrator("Chápem, že je ťažké pochopiť takúto situáciu,", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," ale každopádne,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(mc," ci už sen alebo realita,", 100); FunctionDef.skip(250);
				cout << "\n";
				FunctionDef.narrator("musíme sa pohnúť. Takže by som ti navrhoval vsať skôr než nás niekto", 125); FunctionDef.skip(400);FunctionDef.narratorclean(vc," alebo horšie,",80); FunctionDef.skip(300);FunctionDef.narratorclean(vc," nieco,", 80 ); FunctionDef.skip(250);FunctionDef.narratorclean(vc," nájde.", 80 );
				cout << "\n";
				FunctionDef.narrator("Medzitým, ti skúsim vysvetliť, co som myslel pod \"smrťou\" a taktiež skúsime zistiť, preco som v tvojej hlave", 80);
				cout << "\n";
				...
				if(diadec4=="3"){
					...
					cout << "\n";
					FunctionDef.narrator("Vysvetlím ti všetko, co viem, akonáhle sa odtiaľto dostaneme.", 80); FunctionDef.skip(1500); relwvoid = 3;
				}
				if(diadec4=="4"){
					FunctionDef.MCSC(mc,"Dobre, mali by sme sa pohnúť.", 80); FunctionDef.skip(1000);
					cout << "\n";
				...
				cout << "\n";
				FunctionDef.narrator("musíme sa pohnúť. Takže by som ti radil vstať a odísť odtiaľto skôr než nás niekto", 125); FunctionDef.skip(400);FunctionDef.narratorclean(vc," alebo horšie,",80); FunctionDef.skip(300);FunctionDef.narratorclean(vc," nieco,", 80 ); FunctionDef.skip(250);FunctionDef.narratorclean(vc," nájde.", 80 );
				cout << "\n";
				FunctionDef.narrator("Medzitým, ti skúsim vysvetliť, co som myslel pod \"death\" a taktiež skúsime prísť na to, preco som v tvojej hlave", 80);
				cout << "\n";
				FunctionDef.narrator("a preco si nic nepamätáš.", 80);
				cout << endl << endl;FunctionDef.truenarrator("1) Preco by som ťa mal pocúvať?", 50); cout << "\n";FunctionDef.truenarrator("2) Neverím ti.",50); cout << "\n";FunctionDef.truenarrator("3) Toto je nocná mora, že?",50); cout << "\n";FunctionDef.truenarrator("4) Dobre, mali by sme sa pohnúť.",50); 		
				cout << "\n\n\033[33m>>    ";
				...
				if(diadec4=="2"){
					FunctionDef.MCSC(mc,"Neverím ti.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Nemusíš mi veriť.", 75); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " Stací, keď pohneš s tým svojím zadkom skôr, než nás nieco zabije.", 75); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.MCS("Fajn.", 100); FunctionDef.skip(1500); relwvoid = 2;
				}
				if(diadec4=="3"){
					FunctionDef.MCSC(mc,"Toto je nocná mora, že?", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Ale no tak, vzchop sa.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," Jasné, že toto nie je nocná mora.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," Aj keď možno je, ale na tom nezáleží.", 80); FunctionDef.skip(200); cout << "\n";
					FunctionDef.narrator("Umierať v sne tiež stojí za dve veci.", 80);  FunctionDef.skip(400);FunctionDef.narratorclean(vc," Tak sa už prosím ťa postav a spolu to nejako rozlúskneme.", 80); FunctionDef.skip(1000); cout << "\n";
					FunctionDef.MCS("Dobre, vyhoviem ti.", 80); FunctionDef.skip(1500); relwvoid = 3;
				}
				if(diadec4=="4"){
					FunctionDef.MCSC(mc,"Dobre, mali by sme sa pohnúť.", 80); FunctionDef.skip(1000);
			...
			cout << "\n";
			FunctionDef.narrator("Netuším.", 80); FunctionDef.skip(500);FunctionDef.narratorclean(vc," Ale co viem,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc," je to,", 125);FunctionDef.narratorclean(vc," že by sme mali odtiaľto vypadnúť,", 100); FunctionDef.skip(500); cout << "\n";FunctionDef.narrator("nechceme, aby nás tu cokoľvek našlo, že?", 80); FunctionDef.skip(1000);FunctionDef.narratorclean(vc," Odpovede ti dám po ceste,", 80); FunctionDef.skip(250); cout << "\n";FunctionDef.narrator("proste sa postav a daj do pohybu,", 80); FunctionDef.skip(200);FunctionDef.narratorclean(vc, " prosím.", 100); FunctionDef.skip(1000);
			cout << endl << endl;FunctionDef.truenarrator("1) Preco by som ťa mal pocúvať?", 50); cout << "\n";FunctionDef.truenarrator("2) Neverím ti.",50); cout << "\n";FunctionDef.truenarrator("3) Toto je nocná mora, že?",50); cout << "\n";FunctionDef.truenarrator("4) Dobre, mali by sme sa pohnúť.",50); 		
			cout << "\n\n\033[33m>>    ";
			diadec3 = FunctionDef.GVIclean(1,4);
			cout << "\033[0m";
			...
			if(diadec3=="3"){
				...
				cout << "\n";
				FunctionDef.narrator("Ale no tak, vzchop sa.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," Jasné, že toto nie je nocná mora.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," Aj keď, možno je, ale na tom nezáleží.", 80); FunctionDef.skip(200); cout << "\n";
				FunctionDef.narrator("Umierať v sne tiež za moc nestojí.", 80);  FunctionDef.skip(400);FunctionDef.narratorclean(vc," Tak sa už prosím ťa postav a spolu to nejako rozlúskneme.", 80); FunctionDef.skip(1000); cout << "\n";
				FunctionDef.MCS("Dobre, vyhoviem ti.", 80); FunctionDef.skip(1500); relwvoid = 3;
			}
			if(diadec3=="4"){
				FunctionDef.MCSC(mc,"Dobre, mali by sme sa pohnúť.", 80); FunctionDef.skip(1000);
		...
		if(diadec2=="3"){
			...
			cout << "\n";
			FunctionDef.narrator("Ja som tvojim spomienkam nic neurobil.", 80); FunctionDef.skip(500);FunctionDef.narratorclean(vc," Nic o tom neviem,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc," rovnako ako ty.", 125); FunctionDef.skip(400);FunctionDef.narratorclean(vc," myslíš, že tu chcem byť?", 100); FunctionDef.skip(500); cout << "\n";FunctionDef.narrator("Umrel som ani neviem ako a zrazu som v cudzom tele,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " v tvojom tele,", 100);FunctionDef.narratorclean(vc, " ktoré nemôžem kontrolovať.", 100); FunctionDef.skip(250); cout << "\n";FunctionDef.narrator("Ale,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc," sme tam, kde sme,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc," tak pohni s tým svojím zadkom a postav sa.", 80); FunctionDef.skip(1000);
			cout << endl << endl;FunctionDef.truenarrator("1) Preco by som ťa mal pocúvať?", 50); cout << "\n";FunctionDef.truenarrator("2) Neverím ti.",50); cout << "\n";FunctionDef.truenarrator("3) Toto musí byť nocná mora.",50); cout << "\n";FunctionDef.truenarrator("4) Dobre, mali by sme sa pohnúť.",50); 	
			cout << "\n\n\033[33m>>    ";
			diadec3 = FunctionDef.GVIclean(1,4);
			cout << "\033[0m";
			if(diadec3=="1"){
			...
			if(diadec3=="3"){
				FunctionDef.MCSC(mc,"Toto musí byť nocná mora!...", 80);FunctionDef.narratorclean(mc, "ARGHHHH!", 80); FunctionDef.skip(1000);
				cout << "\n";
				...
				if(diadec4=="2"){
					FunctionDef.MCSC(mc,"Pockaj, preco by som mal umrieť?", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Pretože budeš na tomto svete sám,", 75); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " bez spomienok,", 75); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " ukradtnutý každému,", 75); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " a ľahký cieľ pre nepriateľov.", 75); FunctionDef.skip(250); cout << "\n";FunctionDef.narrator("V tomto lese nie sme sami.", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " A ja som ten, ktorý ti pomôže sa odtiaľto bezpecne dostať.", 75); FunctionDef.skip(1000); 
					cout << "\n";
				...
				if(diadec4=="4"){
					...
					cout << "\n";
					FunctionDef.narrator("Povedal som ti všetko, co viem.", 80);  FunctionDef.skip(500);FunctionDef.narratorclean(vc," Tiež by som rád vedel viac,", 75); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " ale nic nezistíme, pokiaľ tu budeme len tak sedieť.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.MCS("Máš pravdu,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc," poďme odtiaľto prec.", 75); FunctionDef.skip(1500);	relwvoid = 3;
					earlyexplanation = true;	
				}
			}
		}
			...
			cout << "\n";
			FunctionDef.narrator("Nie som klamár.", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " Iba vtedy, keď je to fakt nevyhnutné.", 80); FunctionDef.skip(250); cout << "\n";FunctionDef.narrator("Ale zato ty by si sa mal pohnúť s tým zadkom, inak kvôli tebe obidvaja umrieme.", 80); FunctionDef.skip(1000);
			...
			if(diadec3=="1"){
				...
				cout << "\n";
				FunctionDef.narrator("Povedal som ti,", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " fakt neviem.", 80);FunctionDef.narratorclean(vc, " Jediné co viem je, že som uväznený v tvojej hlave a že nás dostaneš ", 80); cout << "\n";FunctionDef.narrator("do hrobu pokiaľ sa odtiaľto nepohneš,",80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " tak ťa prosím,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " pohni sa,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " a poviem ti viac po ceste.", 80); FunctionDef.skip(1000); 
				...
				if(diadec4=="1"){
					...
					if(diadec5=="1"){
						...
						cout << "\n";
						FunctionDef.narrator("Vieš co, fajn, choď kam chceš, uvidíme, kam sa dostaneš.", 80);
						// endingplaceholder1();
					}
					else{
						...
						cout << "\n";
						FunctionDef.narrator("Tak predsa len.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Nuž,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " nemáme casu nazvyš,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " povstaň a vydaj sa tamtou cestou.", 80); FunctionDef.skip(1500); relwvoid = 3;
					}
				}
				else if(diadec4=="2"){
					...
					if(diadec5=="1"){
						...
						cout << "\n";
						FunctionDef.narrator("Vieš co, ako chceš, choď svojou cestou. Aspoň uvidíme, kam to vedie.", 80);
						// endingplaceholder1();
					}
					else{
						FunctionDef.MCSC(mc, "Vieš co,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " fajn,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " pocúvnem ťa.", 80); FunctionDef.skip(1000);
						cout << "\n";
						FunctionDef.narrator("Konecne...", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Môžeme sa pohnúť.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Postav sa,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " a poďme touto cestou.", 80); FunctionDef.skip(1500); relwvoid = 1;
					}
				}
				else{
				...
				if(diadec4=="1"){
					...
					cout << "\n";
					FunctionDef.narrator("V pohode", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " poďme tadiaľto.", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " Zdá sa mi, že tam nieco vidím.", 80); FunctionDef.skip(1000); relwvoid = 3;
				}
				else{
					FunctionDef.MCSC(mc, "Co ak,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " nie?", 100); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Fakt si chceš zo mňa urobiť nepriateľa?", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Pamätaj,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " som v tvojej hlave,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " a kedykoľvek môžem pre teba z tohto urobiť ", 80); cout << "\n";FunctionDef.narrator("peklo na Zemi.", 80); FunctionDef.skip(1000);
					cout << endl << endl;FunctionDef.truenarrator("1) Áno. A chcem ísť svojou cestou.", 50); cout << "\n";FunctionDef.truenarrator("2) Zmenil som názor, prepác, už ťa budem pocúvať.", 50); 
					cout << "\n\n\033[33m>>    ";
					diadec5 = FunctionDef.GVIclean(1,2);
					cout << "\033[0m";
					if(diadec5=="1"){
						...
						FunctionDef.MCSC(mc, "Zmenil som názor,",80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " prepác,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " už ťa budem pocúvať.", 80); FunctionDef.skip(1000);
						cout << "\n";
						FunctionDef.narrator("Tak nakoniec predsa len,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " fajn,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc, "poďme tadiaľto,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " Skúsim ti vysvetliť všetko, co viem.", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " Pokiaľ chceme spolu nažívať", 80); cout << "\n";FunctionDef.narrator("v mieri,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " tak musíme spolupracovať.", 80); FunctionDef.skip(1500); relwvoid = 2;
					}
				}
			}
			else{
				FunctionDef.MCSC(mc, "Na to ti neskocím.", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " Snažíš sa ma dostať von.", 80); FunctionDef.skip(1000);
				cout << "\n";
				FunctionDef.narrator("Vážne?", 75); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " Pocúvaj,", 80);FunctionDef.narratorclean(vc," nemáš na výber,", 80);FunctionDef.narratorclean(vc," pohni sa,",100);FunctionDef.narratorclean(vc," nemáš co stratiť.", 80); FunctionDef.skip(1000); 
				cout << endl << endl;FunctionDef.truenarrator("1) Prestaň na mňa hovoriť, chcem ísť vlastnou cestou.", 50); cout << "\n";FunctionDef.truenarrator("2) Fajn.", 50); cout << "\n";FunctionDef.truenarrator("3) Prepác, chcel som sa iba uistiť. Môžeme ísť.", 50); 
				cout << "\n\n\033[33m>>    ";
				diadec4 = FunctionDef.GVIclean(1,3);
				cout << "\033[0m";
				if(diadec4=="1"){
					...
					cout << "\n";
					FunctionDef.narrator("Vieš co, fajn, choď, kadiaľ chceš, uvidíme, co z tohto vznikne.", 80); FunctionDef.skip(1500);
					// endingplaceholder1();
				}
				else if(diadec4=="2"){
					FunctionDef.MCSC(mc, "Fajn.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Tvoj postoj je otrasný,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " mohol by si prejaviť aspoň trochu úcty tomu hlasu, co máš v hlave.", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, "Poobzeraj sa trochu,", 80); FunctionDef.skip(250); cout << "\n";FunctionDef.narrator("vyzerá to, že tamtým smerom nieco je.", 80); FunctionDef.skip(1500); relwvoid = 1;
				}
				else{
					FunctionDef.MCSC(mc, "Prepác,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " chcel som sa len uistiť.", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " Môžeme sa dať do pohybu.", 80); FunctionDef.skip(1000);
					cout << "\n";
		...
		while(true){
			...
			if(decision1=="1"){
				...
				if(decision1sub1=="1"){
					...
					cout << endl << endl;
					FunctionDef.truenarrator("Nuž dobre, rozhodol si sa vydať na dobrodrúžnu cest- hups, tak nic, umrel si. Dôvod: NEZNÁMY", 50);
					FunctionDef.skip(400);
					lives--; deathsequence(lang);
				}
				else if(decision1sub1=="2"){
		...
		if(b0=="1"){
			...
			if(diadec1=="1"){
				...
				cout << endl << endl;FunctionDef.truenarrator("Zobral si veci a odteraz ho môžeš nájsť vo svojom inventári, ktorým je batoh.", 50); FunctionDef.skip(500); cout << "\n";
			}
			else{
				...
				cout << "\n";
				FunctionDef.narrator("Ja?!", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Vieš co,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " choď do certa,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " povedal som ti, co sa stalo a preco som tu.", 80); FunctionDef.skip(400); cout << "\n";FunctionDef.narrator("Tak láskavo prestaň, zober veci a padaj prec odtiaľto skôr, než umrieme.", 80); FunctionDef.skip(1000);
			...
			FunctionDef.narrator("Fakt sa snažím byť milý,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " ale ty ma proste nenecháš.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Co navrhuješ teraz?", 80); FunctionDef.skip(400); cout << "\n";FunctionDef.narrator("Pôjdeme touto cestou?", 80); FunctionDef.skip(1000);
			cout << endl << endl;FunctionDef.truenarrator("1) Ísť cestou, ktorú navrhol Void.", 80); cout << "\n";FunctionDef.truenarrator("2) Ísť opacným smerom.", 50); cout << endl << endl;
			cout << "\033[33m>>    ";
			b1pathdec = FunctionDef.GVIclean(1,2);
			cout << "\033[0m";
			if(b1pathdec=="1"){
				...
				cout << "\n";
				FunctionDef.narrator("Aspoň že pocúvaš.", 80); FunctionDef.skip(1000); relwvoid = 2;
			}
			else{
				FunctionDef.MCSC(mc, "Vieš co?", 80); FunctionDef.skip(400);FunctionDef.narratorclean(mc, "nechce sa mi ťa pocúvať.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(mc, "Radšej by som šiel tdiaľto,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " nemáme co stratiť.", 80); 
				cout << "\n";
				FunctionDef.narrator("Vieš co,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " fajn,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " choď do ktorejkoľvek riti chceš.", 80); FunctionDef.skip(1000); relwvoid = 1; d="7";
			}
		}
		else if(b0=="2"){
			FunctionDef.narrator("Preco by si si vybral stromy 3-krát po sebe?", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Nie si náhodou", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, "-", 200);FunctionDef.narratorclean(vc, " Ospravedlňujem sa.", 100); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Tvoj výber ma frustroval.", 80); FunctionDef.skip(400); cout << "\n";FunctionDef.narrator("Je úplne jasné, že tam nic nie je,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " strácame náš cas.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Mali by sme ísť ďalej,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " zobver veci a poď.", 80); FunctionDef.skip(1000);
			cout << "\n"; 
			FunctionDef.MCS("Prepác.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(mc, " Poďme tú cestu, co sme videli predtým.", 80); FunctionDef.skip(1000);
			...
			FunctionDef.narrator("Zober to a poďme.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " A inak,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " co je to s tebou?", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Upusti od takýchto rozhodnutí,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " bude lepšie, keď sa dohodneme.", 80); FunctionDef.skip(1000);
			cout << endl << endl;FunctionDef.truenarrator("Zobral si veci a odteraz ho môžeš nájsť vo svojom inventári, ktorým je batoh.", 50); FunctionDef.skip(500);
			...
			cout << "\033[0m";			 
			if(diadec1=="1"){
				FunctionDef.MCSC(mc, "Ospravedlňujem sa za to...", 80); FunctionDef.skip(400);FunctionDef.narratorclean(mc, " Nemyslel som to tak,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " bol som iba opatrný.", 80); FunctionDef.skip(400); cout << "\n";FunctionDef.MCS("But Ale aspoň už vidím, že nie si zlý a že si naozaj v mojej hlave.", 80); FunctionDef.skip(1000);
				cout << "\n";
				...
				FunctionDef.MCSC(mc, "...", 80); FunctionDef.skip(1000);
				cout << "\n";
				FunctionDef.narrator("Ja-", 150); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " chápem, že to musí byť z tvojho pohľadu ťažké.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Keď sa na to pozerám z tvojej perspektívy,", 80);FunctionDef.narratorclean(vc, " tiež by som sebe neveril.", 80); FunctionDef.skip(400); cout << "\n";FunctionDef.narrator("Chápem ťa,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " prepác.", 100); FunctionDef.skip(1000);
				cout << "\n";
			...
			FunctionDef.narrator("Výborne.", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc," Tie veci môžu byť pre nás užitocné.", 80); FunctionDef.skip(400); FunctionDef.narratorclean(vc, " Zober ich a poďme ďalej.", 80); FunctionDef.skip(1000);
			cout << endl << endl;FunctionDef.truenarrator("Zobral si veci a odteraz ho môžeš nájsť vo svojom inventári, ktorým je batoh.", 50); FunctionDef.skip(500);
			cout << endl << endl;
			...
			FunctionDef.narrator("Vypadneme odtiaľto,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " tamtou cestou.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Choď a nic nehovor.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Tvoja rec ma zacína iritovať", 80); cout << "\n";FunctionDef.narrator("a to sme sa spoznali len 3 minúty dozadu.", 80); FunctionDef.skip(1000);			
		}
		else if(b0=="7"){
			FunctionDef.narrator("Super,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " niekam sa dostávame.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Ale preco si si 2-krát vybral stromy?", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Príde mi to dosť mätúce,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " ale co už,", 80); FunctionDef.skip(250); cout << "\n";FunctionDef.narrator("mali by sme pojracovať ďalej,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " môžeme ísť tou cestou, ktorú sme videli predtým.", 80); FunctionDef.skip(1000);
			cout << "\n";
			cout << endl << endl;FunctionDef.truenarrator("Zobral si veci a odteraz ho môžeš nájsť vo svojom inventári, ktorým je batoh.", 50); FunctionDef.skip(500);
			cout << "\n";
			...
			FunctionDef.narrator("Výborne,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " zober ich,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " môžu byť užitocné.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " A hneď potom,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " by sme sa mali dať do pohybu,", 80); FunctionDef.skip(250); cout << "\n";FunctionDef.narrator("môžeme ísť tou cestou, ktorú sme videli predtým.", 80); FunctionDef.skip(1000);
			cout << "\n";
			...
			cout << endl << endl;FunctionDef.truenarrator("Zobral si veci a odteraz ho môžeš nájsť vo svojom inventári, ktorým je batoh.", 50); FunctionDef.skip(500);
			cout << "\n";FunctionDef.narrator("Myslím, že je to naša jediná možnosť.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Musí viesť do dediny,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " na sto percent.", 80); FunctionDef.skip(1000);			 
		}
		if(b0!="1"&&relwvoid!=1){
			cout << endl << endl;FunctionDef.truenarrator("Si obkľúcený svorkou vlkov, ktorá je prepravená na teba zaútociť. Musíš sa brániť.", 50); cout << "\n";FunctionDef.truenarrator("Musíš sa rýchlo rozhodnúť, co urobíš: ", 50); cout << endl << endl;FunctionDef.truenarrator("1) POužiť palicu a bojovať s nimi",50); FunctionDef.skip(200); cout << "\n";FunctionDef.truenarrator("2) Použiť mec a bojovať s nimi", 50); cout << "\n";FunctionDef.truenarrator("3) Skúsiť ich odlákať palicou namiesto bojovania.", 50); cout << endl << endl;
			cout << "\033[33m>>    ";
			decision2 = FunctionDef.GVI(1,3);
			cout << "\033[0m" << endl;
			if(decision2=="1"){
				...
				FunctionDef.truenarrator("Rozhodol si sa zobrať mec a s ním sa vrhnúť do boja. Zahájujem súboj.", 50); FunctionDef.skip(200);
				ambush("Wolves", lang); 
			}
			else if(decision2=="3"){
				int wolfescapechance = rand()%4;
				if(wolfescapechance==1){
					...
					FunctionDef.truenarrator("Skúsil si vlkov odlákať palicou, ale vôbec nezareagovali a roztrhali ťa ka kúsky.", 50); FunctionDef.skip(400); cout << "\n";
					lives--; deathsequence(lang);
				}
			}
		}
		...
		if(c=="1"){
			FunctionDef.narrator("Zbláznil si sa?", 80); FunctionDef.skip(400); FunctionDef.narratorclean(vc, " Mohol si nás zabiť.", 80); FunctionDef.skip(400); FunctionDef.narratorclean(vc, " Máš šťastie, že to vyšlo.", 80); FunctionDef.skip(400); cout << "\n"; FunctionDef.narrator("No nic, pokracujme ďalej po ceste.", 80); FunctionDef.skip(1000);
			cout << "\n"; 
			FunctionDef.MCS("Bol to dobrý nápad,", 80); FunctionDef.skip(250); FunctionDef.narratorclean(mc, " v boji by sme mohli umrieť.", 80); FunctionDef.skip(400); FunctionDef.narratorclean(mc, " Odlákal som ich a teraz sme v bezpecí.", 80); FunctionDef.skip(400); cout << "\n"; FunctionDef.MCS("Idem teda ďalej.", 80); FunctionDef.skip(1000);
		}
		else if(c=="2"){
			FunctionDef.narrator("Wau!", 80); FunctionDef.skip(400); FunctionDef.narratorclean(vc, " To s tou palicou bol dobrý nápad,", 80); FunctionDef.skip(250); FunctionDef.narratorclean(vc, " necakal som, že to bude fungovať.", 80); FunctionDef.skip(1000);
			cout << "\n";
			...
			FunctionDef.narrator("Preco by si používal na vlkov palicu?", 80); FunctionDef.skip(400); FunctionDef.narratorclean(mc, " Máme šťastie, že sme stále nažive.", 80); FunctionDef.skip(1000);
			cout << "\n"; 
			FunctionDef.MCS("Bolo to veľmi spontánne rozhodnutie.", 80); FunctionDef.skip(400); FunctionDef.narratorclean(mc, " Možno to bolo ubáhlené,", 80); FunctionDef.skip(250); FunctionDef.narratorclean(mc, " ale každopádne sme vyhrali.", 80); FunctionDef.skip(400); FunctionDef.narratorclean(mc, " Pokracujme ďalej.", 80); FunctionDef.skip(1000); 
		}
		else if(c=="4"){
			FunctionDef.narrator("Zaujímavé...?", 100); FunctionDef.skip(400); FunctionDef.narratorclean(vc, " Dobrá práca,", 80); FunctionDef.skip(250); FunctionDef.narratorclean(vc, " ocividne vieš, ako používať palicu.", 80); FunctionDef.skip(400); FunctionDef.narratorclean(vc, " Ale preco si namiesto toho nepoužil mec?", 80); FunctionDef.skip(1000);
			cout << "\n";
			...
			FunctionDef.narrator("Aspoň že vieš bojovať s mecom.", 80); FunctionDef.skip(400); FunctionDef.narratorclean(vc, " Pokracuj smerom vpred.", 80); FunctionDef.skip(1000);
			cout << "\n";
			FunctionDef.MCS("Ďakujem za uznanie, asi...", 80); FunctionDef.skip(400); FunctionDef.narratorclean(mc, " Zatiaľ si mi nebol veľmi nápomocný,", 80); FunctionDef.skip(250); FunctionDef.narratorclean(mc, " vieš o tom?", 80); FunctionDef.skip(400); cout << "\n"; FunctionDef.MCS("Ale to asi nie je tvoja chyba.", 80); FunctionDef.skip(400); FunctionDef.narratorclean(mc, " Pokracujme ďalej.", 80); FunctionDef.skip(400);
		}
		else if(c=="6"){
			FunctionDef.narrator("Úžasné,", 80); FunctionDef.skip(250); FunctionDef.narratorclean(vc, " bol si super!", 80); FunctionDef.skip(400); FunctionDef.narratorclean(vc, " Mali by sme sa pohnúť ďalej skôr, než sem príde ďalšia svorka.", 80); FunctionDef.skip(400); cout << "\n"; FunctionDef.narrator("Som si istý, že je ich tu veľa.", 80); FunctionDef.skip(1000);
			cout << "\n";
