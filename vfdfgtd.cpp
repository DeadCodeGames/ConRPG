...
public:
	...
	void checkLevelUp(string language){
		...
		if(language=="en"){
			...
			cout << "\n      Zv˝öila sa it ˙roveÚ " << initiallvl << " na ˙roveÚ " << level << ". HP: " << currenthp << " -> " << hp << "\n";
	...
	void deathsequence(string language){
		if(language=="en"){
			...
			system("cls");
			cout << "\n\n"; FunctionDef.truenarrator("Ups...", 50); FunctionDef.skip(200); FunctionDef.TNC(" Vypad· to trochu tak, ûe niekto priöiel o jeden ûivot.", 50); FunctionDef.skip(200); cout << "\n";
			if(lives!=0){
				FunctionDef.truenarrator("\tNemusÌö sa ale b·ù, odv·ûny cestovateæ!", 50); FunctionDef.skip(200); FunctionDef.TNC(" Smrù je sk˙senosù a sk˙senosù je ûivot.", 50); FunctionDef.skip(200); 
				if(lives==2) FunctionDef.TNC("Ost·vaju ti " + to_string(lives) + " ûivoty", 50);
				else if(lives==1) FunctionDef.TNC("Ost·va ti 1 ûivot.", 50);
				else{
					FunctionDef.TNC("M·ö nekonecno ûivotov.", 50);
				}
				cout << "\n"; FunctionDef.skip(250);
				FunctionDef.truenarrator("\t[Stlac F na oûivenie, L na zacatie novej hry.]", 50);
				while(true){
					if(_kbhit()){
				  	char key = _getch();
				  	if(key=='f'){
				...
				FunctionDef.truenarrator("\tA bol to tvoj posledn˝ ûivot...", 50); FunctionDef.skip(250); FunctionDef.TNC(" Je mi æ˙to ûe sa ti toto stalo.", 50); FunctionDef.skip(250);
				cout << "\n";
	...
	if (CreateProcess(NULL, commandLine, NULL, NULL, FALSE, CREATE_NEW_CONSOLE, NULL, NULL, &si, &pi)) {
			...
			cout << "Chyba pri spustenÌ programu." << endl;
		}
		ifstream inputFile("testsave.txt");
	  if (!inputFile.is_open()) {
	    cerr << "Chyba pri otvorenÌ s˙boru na nacÌtanie ˙dajov." << endl;
	    return 1;
	  }
	  inputFile >> fightstatus;
	  inputFile >> xpgained;
	  if(fightstatus=="1"){
	  	FunctionDef.truenarrator("Preruöil si s˙boj... Automaticky umieraö.", 50); cout << "\n"; FunctionDef.skip(250); FunctionDef.truenarrator("[Stlac F pre nacÌtanie poslednej uloûenej pozÌcie alebo stlac L pre nacÌtanie novej hry.]",50);
			while(true){
      	if(_kbhit()){
      		char c = _getch();
	...
	void endingplaceholder1(string language){
		if(language=="en"){
			...
			cout << "\n\n";
			FunctionDef.truenarrator("Iöiel si vlastnout cestou. Nikdy sa nedozvieö co sa stalo.", 50); FunctionDef.skip(250); cout << "\n\n";
	...
	void d1n(string language) {
		if(language=="en"){
			...
			if (a == 0) FunctionDef.truenarrator("\t  Stromy, zaujÌmav· voæba. Iöiel si bliûöie ku stromom, ale nic si nenaöiel.", 50);
			if (a == 1) FunctionDef.truenarrator("\t  V·ûne? Priöiel si znova ku stromom, a... znova si nic nenaöiel.", 50);
			if (a == 2) FunctionDef.truenarrator("\t  Nieco je divnÈ na t˝ch stromoch. S˙ ako... stromy.", 50);
			if (a > 2) {
				FunctionDef.truenarrator("\t  Iöiel si k stromom eöteb raz.A op‰ù si nic nenaöie-", 50); cout << "\n";
				FunctionDef.truenarrator("\t  C- co to je za zvuk? Vyzer· to tak... ûe si nieco zobudil. A je to naötvanÈ.", 75); cout << "\n"; FunctionDef.skip(300);
				FunctionDef.truenarrator("\t  Axir, The Lord of the Forbidden Forest [BOSS] [10M HP] Nem·ö öancu proti tomuto monötru zvÌùaziù, keÔûe ùa zabije", 50); cout << "\n";
				FunctionDef.truenarrator("\t  eöte predt˝m, neû m· s˙boj vÙbec öancu zacaù.", 50); FunctionDef.skip(400); cout << "\n"; FunctionDef.skip(400); system("cls"); cout << "\n\n\n\n";
				FunctionDef.truenarrator("\t  \033[1;31mZÌskal si koniec Zberac Stromov.\033[0m", 50); cout << "\n"; FunctionDef.skip(400); cout << "\n\n\n\n\n\n\n\n\n\n";
				lives--; UlozitInfo(to_string(lives), "Lives.txt"); deathsequence(lang);
			}
		}
	}
	...
	void d1(string language) {
		if(language=="en"){
			...
			FunctionDef.truenarrator("Poobzer·ö sa okolo seba a vidÌö:", 50);
			cout << "\n";
			FunctionDef.truenarrator("1) Stromy", 50); cout << "\n"; FunctionDef.truenarrator("2) Stromy", 50); cout << "\n"; FunctionDef.truenarrator("3) Stromy", 50); cout << "\n"; FunctionDef.truenarrator("4) Cestu", 50); cout << "\n"; FunctionDef.truenarrator("5) DivnÈ miesto medzi stromami, kde mÙûeö vidieù pohodenÈ veci.", 50); FunctionDef.skip(500); cout << "\n\n"; FunctionDef.truenarrator("What do you choose?", 50);
			cout << "\n";
		}
	}
	...
	void savesequence(string language) {
		if(language=="en"){
			...
			FunctionDef.truenarrator("\tVitaj sp‰ù, " + playerName, 50); FunctionDef.skip(250); cout << "\n";
			FunctionDef.truenarrator("\tChceö nacÌtaù posledn˙ uloûen˙ pozÌciu alebo zacaù nov˙ hru? [F pre nacÌtanie pozÌcie, L pre nov˙ hru]", 50);
		}
		while (true) {
			if (_kbhit()) {
	...
	void introsequence(string language){
		...
		cout << "                                            V I T A J    V" << endl;
	 	cout << " .----------------.    .----------------.  .----------------.  .-----------------.   .----------------. " << endl << "| .--------------. |  | .--------------. || .--------------. || .--------------. |  | .--------------. |" << endl << "| |              | |  | |     ______   | || |     ____     | || | ____  _____  | |  | |              | |" << endl << "| |              | |  | |   .' ___  |  | || |   .'    `.   | || ||_   \\|_   _| | |  | |              | |" << endl << "| |    ______    | |  | |  / .'   \\_|  | || |  /  .--.  \\  | || |  |   \\ | |   | |  | |    ______    | |" << endl << "| |   |______|   | |  | |  | |         | || |  | |    | |  | || |  | |\\ \\| |   | |  | |   |______|   | |" << endl << "| |              | |  | |  \\ `.___.'\\  | || |  \\  `--'  /  | || | _| |_\\   |_  | |  | |              | |" << endl << "| |              | |  | |   `._____.'  | || |   `.____.'   | || ||_____|\\____| | |  | |              | |" << endl << "| |              | |  | |              | || |              | || |              | |  | |              | |" << endl << "| '--------------' |  | '--------------' || '--------------' || '--------------' |  | '--------------' |" << endl << " '----------------'    '----------------'  '----------------'  '----------------'    '----------------' ";
		cout << endl << "                                           The C++ Console RPG";
		cout << endl << "                                         Stlac [cokoævek] a zacni hru"; 
		_getch();
		cout << "\n";FunctionDef.truenarrator("ZdravÌm ùa cestovateæ!", 100); FunctionDef.skip(1000); cout << "\n";
		FunctionDef.truenarrator("...", 500); FunctionDef.skip(1000); cout << "\n";
		FunctionDef.truenarrator("Asi sa nevol·ö cestovateæ, ûe", 80); FunctionDef.skip(1500); cout << "\n";
		FunctionDef.truenarrator("Ako sa vol·ö?", 80); FunctionDef.skip(1000); cout << endl << "      [NAPÕä SVOJE MENO]" << endl;
		FunctionDef.flushInput();
		cout << ">>    "; getline(cin, input); FunctionDef.skip(1000); playerName = input; save();
		if (!playerName.empty()) {
			FunctionDef.truenarrator("R·d ùa spozn·vam, "+playerName+"!", 100);
		} else {
			FunctionDef.truenarrator("Ty...", 500); FunctionDef.TNC(" ty nem·ö meno?", 125); FunctionDef.skip(500); FunctionDef.TNC(" Isto m·ö nejakÈ meno. D·m ti öancu ho eöte zadaù..", 80); cout << "\n"; 
			FunctionDef.flushInput();
			cout << ">>    "; getline(cin, input); FunctionDef.skip(1000); playerName = input; save();
			if (!playerName.empty()) {
			FunctionDef.truenarrator("V˝borne, r·d ùa spozn·vam, "+playerName+"!", 80);
		} else {
			FunctionDef.truenarrator("V·ûne nem·ö meno?", 125); cout << "\n"; FunctionDef.skip(1000);
			FunctionDef.truenarrator("To je", 225); FunctionDef.TNC("...", 500); FunctionDef.skip(500); FunctionDef.TNC(" smutnÈ. Je mi ùa æ˙to.", 200); cout << "\n"; FunctionDef.skip(750);
			FunctionDef.truenarrator("Ale aj tak ùa nejako potrebujem volaù.", 80); cout << "\n"; FunctionDef.skip(500);
			FunctionDef.truenarrator("MÙûem ùa volaù Cestovateæ?", 75); FunctionDef.skip(500); FunctionDef.TNC(" MyslÌm, ûe to budem fajn.", 75); playerName = "Cestovateæ"; save();
		}
	} 
				FunctionDef.skip(1000); cout << "\n";
			FunctionDef.truenarrator("Na akej obtiaûnosti si prajeö hraù hru, drah˝ cestovateæ?", 80); FunctionDef.skip(200); cout << "\n"; FunctionDef.truenarrator("<1> ºAHK¡", 50); FunctionDef.skip(200); cout << "\n"; FunctionDef.truenarrator("<2> NORM¡LNA", 50); FunctionDef.skip(200); cout << "\n"; FunctionDef.truenarrator("<3> çAéK¡", 50); FunctionDef.skip(200); cout << "\n"; FunctionDef.truenarrator("<4> TURBO", 50);
			mode = FunctionDef.GVIclean(1,4);
			if(mode=="1") mode = "easy";
			else if(mode=="2") mode = "normal";
			else if(mode=="3") mode = "hard";
			...
			system("cls");
			if(mode=="easy"){
				cout << "\n\n\n\n\n\n"; FunctionDef.truenarrator("\t\tJednoduch· obtiaûnosù...", 100); FunctionDef.skip(250);
				cout << "\n\n"; FunctionDef.truenarrator("\t\tM·ö neobmedzen˝ pocet oûivenÌ.", 50); 
				Sleep(2000); lives = -1;
			}
			else if(mode=="normal"){
				cout << "\n\n\n\n\n\n"; FunctionDef.truenarrator("\t\tNorm·lna obtiaûnosù...", 100); FunctionDef.skip(250);
				cout << "\n\n"; FunctionDef.truenarrator("\t\tM·ö k dispozÌcii 3 oûivenia.", 50); 
				Sleep(2000); lives = 3;
			}
			else if(mode=="hard"){
				cout << "\n\n\n\n\n\n"; FunctionDef.truenarrator("\t\tç·ûk· obtiaûnosù...", 100); FunctionDef.skip(250);
				cout << "\n\n"; FunctionDef.truenarrator("\t\tM·ö k dispozÌcii jedno oûivenie.", 50); 
				Sleep(2000); lives = 1;
			}
			else{
				cout << "\n\n\n\n\n\n"; FunctionDef.truenarrator("\t\tSpeedrun mode...", 100); FunctionDef.skip(250);
				cout << "\n\n"; FunctionDef.truenarrator("\t\tM·ö jedno oûivenie a hra sa neuklad·. Tvoja cesta bude maù t˙ spr·vnu r˝chlosù.", 50); 
				Sleep(2000); lives = 1;
			}
			UlozitInfo(to_string(lives), "Lives.txt");
			sequence=0;
	...
	void sequence1(string language){
		...
		if(saveload==1){
				cout << "\n\n\n"; FunctionDef.truenarrator("\tVitaj sp‰ù cestovateæ. Ch˝bal si n·m. MÙûeö pokracovaù "+mode+ " na svojej ceste.", 50); Sleep(2500);
				system("cls");
			}
			if(sequence==0) sequence++;
			save();
			Sleep(500);
		cout << "\n\n\n\n\n\n\t\t{[()]} Akt 1 {[()]}\n\n\n"; Sleep(500);
		FunctionDef.truenarrator("\t\tHra automaticky uklad· kaûd˝ akt. Veæa öùastia na cest·ch.", 50);
		Sleep(2000); system("cls");
		Sleep(2500);
		FunctionDef.MCS("Kde to som?", 80);FunctionDef.narratorclean(mc," Ako som sa sem dostal?",80); FunctionDef.skip(1000);
		cout << "\n";
		FunctionDef.MCS("Vöade okolo mÚa je len hust˝ les.", 80); FunctionDef.skip(1000);
		cout << "\n";
		...
		cout << "\n";
		FunctionDef.narrator("Co sa deje?! ", 80); FunctionDef.skip(600);FunctionDef.narratorclean(vc,"Preco nemÙûem ovl·daù svoje telo? ", 100); FunctionDef.skip(500);FunctionDef.narratorclean(vc,"Pockaù, toto... ", 175); FunctionDef.skip(300);FunctionDef.narratorclean(vc,"Toto nie je moje telo!", 80);
		cout << "\n";
		FunctionDef.MCS("Kto to povedal? ", 40); FunctionDef.skip(750);
		cout << "\n";
		FunctionDef.MCS("Kde si? Uk·û sa!", 75); FunctionDef.skip(1000);
		cout << "\n";
		FunctionDef.narrator("NemÙûem sa uk·zaù.", 80); FunctionDef.skip(1000); cout << "\n";
		FunctionDef.MCS("Ako to myslÌö ûe nemÙûeö?", 80); FunctionDef.skip(1000);
		cout << "\n";
		FunctionDef.narrator("S·m netuöÌm.", 80); FunctionDef.skip(500);FunctionDef.narratorclean(vc," Vyzer· to tak...", 120); FunctionDef.skip(400);FunctionDef.narratorclean(vc," ûe som v tebe.", 100); FunctionDef.skip(750);
		cout << endl << endl;FunctionDef.truenarrator("1) Preco si vo mne?", 50); cout << "\n";FunctionDef.truenarrator("2) Ako je to moûnÈ?",50); cout << "\n";FunctionDef.truenarrator("3) Co sa stalo s mojimi spomienkami?",50); cout << "\n";FunctionDef.truenarrator("4) Klameö!. Isto sa schov·vaö niekde za t˝mi stromami.",50); 
		cout << "\n\n\033[33m>>    ";
		diadec2 = FunctionDef.GVIclean(1,4);
		cout << "\033[0m";
		if(diadec2=="1"){
			...
			cout << "\n";
			FunctionDef.narrator("Dobr· ot·zka.", 80); FunctionDef.skip(500);FunctionDef.narratorclean(vc," Vöetko co si pam‰t·m, je moja smrù.", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc," a zrazu,", 125);FunctionDef.narratorclean(vc," som sa ocitol tu,", 100); FunctionDef.skip(500);FunctionDef.narratorclean(vc," v tvojom tele.", 100); FunctionDef.skip(1000);
			cout << endl << endl;FunctionDef.truenarrator("1) Co myslÌö pod tvojou smrùou?", 50); cout << "\n";FunctionDef.truenarrator("2) Pockaj, ty si m‡tvy?",50); cout << "\n";FunctionDef.truenarrator("3) Toto je sen, ûe?",50); cout << "\n";FunctionDef.truenarrator("4) Co?",50); 		
			cout << "\n\n\033[33m>>    ";
			diadec3 = FunctionDef.GVIclean(1,4);
			cout << "\033[0m";
			if(diadec3=="1"){
				FunctionDef.MCSC(mc,"Co myslÌö pod tvojou smrùou?", 80); FunctionDef.skip(1000);
				cout << "\n";
				FunctionDef.narrator("Toto musÌ byù nocn· mora.", 80); FunctionDef.skip(1000);
				cout << "\n";
				FunctionDef.narrator("Nuû,", 125); FunctionDef.skip(400);FunctionDef.narratorclean(vc," nem·me moc na v˝ber,",80); FunctionDef.skip(300);FunctionDef.narratorclean(vc," postav sa a poobzeraj sa tu trochu.", 80 ); FunctionDef.skip(250);
				cout << "\n";
				FunctionDef.narrator("Medzit˝m ti sk˙sim vysvetliù co som myslel pod \"smrùou\" a taktieû sk˙sime zistiù, preco som v tvojej hlave ", 80);
				cout << "\n";
				FunctionDef.narrator("a preco si nic nepam‰t·ö.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," Postav sa a poÔme inde, skÙr neû n·s niekto", 100);FunctionDef.skip(200);FunctionDef.narratorclean(vc," alebo horöie,", 125); FunctionDef.skip(200);FunctionDef.narratorclean(vc," nieco n·jde.", 100); FunctionDef.skip(1000);
				cout << endl << endl;FunctionDef.truenarrator("1) Preco by som ùa mmal poc˙vaù?", 50); cout << "\n";FunctionDef.truenarrator("2) NeverÌm ti.",50); cout << "\n";FunctionDef.truenarrator("3) Toto je nocn· mora,ûe?",50); cout << "\n";FunctionDef.truenarrator("4) Dobre, mali by sme sa pohn˙ù.",50); 		
				cout << "\n\n\033[33m>>    ";
				diadec4 = FunctionDef.GVIclean(1,4);
				cout << "\033[0m";
				if(diadec4=="1"){
					FunctionDef.MCSC(mc,"Preco by som ùa mal poc˙vaù?", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("MyslÌö, ûe m·ö na v˝ber? Co inÈ chceö robiù?", 75); FunctionDef.skip(1500); relwvoid = 2;
				}
				if(diadec4=="2"){
					FunctionDef.MCSC(mc,"NeverÌm ti.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("NemusÌö mi veriù.", 75); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " StacÌ, keÔ sa postavÌö a pohneö t˝m svojim zadkom predt˝m neû n·s nieco n·jde a obidvoch n·s zabije.", 75); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.MCS("Fajn.", 100); FunctionDef.skip(1500); relwvoid = 2;
				}
				if(diadec4=="3"){
					FunctionDef.MCSC(mc,"Toto je nocn· mora,ûe?", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Ale no tak, vschop sa.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," JasnÈ, ûe toto nie je nocn· mora.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," Aj keÔ moûno aj je, ale na tom nez·leûÌ.", 80); FunctionDef.skip(200); cout << "\n";
					FunctionDef.narrator("Umieraù vo sne tieû stojÌ za nic.", 80);  FunctionDef.skip(400);FunctionDef.narratorclean(vc," Tak sa prosÌm ùa postav a spolu na to prÌdeme.", 80); FunctionDef.skip(1000);
					FunctionDef.MCS("Dobre, vyhoviem ti.", 80); FunctionDef.skip(1500); relwvoid = 3;
				}
				if(diadec4=="4"){
					FunctionDef.MCSC(mc,"Oukej, mali by sme sa pohn˙ù.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("V˝borne, poÔme tadiaæto.", 80);  FunctionDef.skip(500);FunctionDef.narratorclean(vc," Vyzer· to tam sæubne.", 75); FunctionDef.skip(1500); relwvoid = 4;
				}
			}
			else if(diadec3=="2"){
				FunctionDef.MCSC(mc,"Pockaù, ty si m‡tvy?", 70); FunctionDef.skip(1000);
				cout << "\n";
				FunctionDef.narrator("Potvrdzujem. Teda aspoÚ to tak vyzer·.", 80); FunctionDef.skip(1000);
				cout << "\n";
				FunctionDef.narrator("No,", 125); FunctionDef.skip(400);FunctionDef.narratorclean(vc," nem·me zrovna cas na zbytocnÈ prem˝öæania,",80); FunctionDef.skip(300);FunctionDef.narratorclean(vc," postav sa a poobzeraj sa.", 80 ); FunctionDef.skip(250);
				cout << "\n";
				FunctionDef.narrator("Medzit˝m, ti sk˙sim vysvetliù co som myslel pod \"smrùou\" a taktieû sk˙sime zistiù, preco som v tvojej hlave", 80);
				cout << "\n";
				FunctionDef.narrator("a preco si nic nepam‰t·ö.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," Postav sa skÙr neû n·s niekto", 100);FunctionDef.skip(200);FunctionDef.narratorclean(vc," alebo horöie,", 125); FunctionDef.skip(200);FunctionDef.narratorclean(vc," nieco n·jde.", 100); FunctionDef.skip(1000);
				cout << endl << endl;FunctionDef.truenarrator("1) Preco by som ùa mal poc˙vaù?", 50); cout << "\n";FunctionDef.truenarrator("2) NeverÌm ti.",50); cout << "\n";FunctionDef.truenarrator("3) Toto je nocn· mora, ûe?",50); cout << "\n";FunctionDef.truenarrator("4) Dobre, mali by sme sa pohn˙ù.",50); 		
				cout << "\n\n\033[33m>>    ";
				diadec4 = FunctionDef.GVIclean(1,4);
				cout << "\033[0m";
				if(diadec4=="1"){
				...
				if(diadec4=="2"){
					FunctionDef.MCSC(mc,"NeverÌm ti.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("NemusÌö mi veriù.", 75); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " StacÌ, keÔ sa pohneö s t˝m svojÌm zadkom predt˝m, neû n·s nieco zabije.", 75); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.MCS("Fajn.", 100); FunctionDef.skip(1500); relwvoid = 2;
				}
				if(diadec4=="3"){
					FunctionDef.MCSC(mc,"Toto je nocn· mora, ûe?", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Ale no tak, vzchop sa.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," JasnÈ, ûe toto nie je nocn· mora.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," Aj keÔ, moûno aj je, ale na tom nez·leûÌ.", 80); FunctionDef.skip(200); cout << "\n";
					FunctionDef.narrator("Umieranie v sne tieû stojÌ za nic.", 80);  FunctionDef.skip(400);FunctionDef.narratorclean(vc," Tak sa prosÌm ùa postav a spolu na to prÌdeme.", 80); FunctionDef.skip(1000);
					FunctionDef.MCS("Dobre, vyhoviem ti.", 80); FunctionDef.skip(1500); relwvoid = 3;
				}
				if(diadec4=="4"){
					FunctionDef.MCSC(mc,"Dobre, mali by sme sa pohn˙ù.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("V˝borne, poÔme tadiaæto.", 80);  FunctionDef.skip(500);FunctionDef.narratorclean(vc," Vyzer· to tam sæubne.", 75); FunctionDef.skip(1500); relwvoid = 4;
				}
			}
			else if(diadec3=="3"){
				FunctionDef.MCSC(mc,"To nie je moûnÈ!", 70);FunctionDef.narratorclean(mc," SnÌvam?", 100); FunctionDef.skip(1000);
				cout << "\n";
				FunctionDef.narrator("Ch·pem, ûe je ùaûkÈ pochopiù tak˙to situ·ciu,", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," ale kaûdop·dne,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(mc," ci uû sen alebo realita,", 100); FunctionDef.skip(250);
				cout << "\n";
				FunctionDef.narrator("musÌme sa pohn˙ù. Takûe by som ti navrhoval vsaù skÙr neû n·s niekto", 125); FunctionDef.skip(400);FunctionDef.narratorclean(vc," alebo horöie,",80); FunctionDef.skip(300);FunctionDef.narratorclean(vc," nieco,", 80 ); FunctionDef.skip(250);FunctionDef.narratorclean(vc," n·jde.", 80 );
				cout << "\n";
				FunctionDef.narrator("Medzit˝m, ti sk˙sim vysvetliù, co som myslel pod \"smrùou\" a taktieû sk˙sime zistiù, preco som v tvojej hlave", 80);
				cout << "\n";
				FunctionDef.narrator("a preco si nic nepam‰t·ö.", 80);
				cout << endl << endl;FunctionDef.truenarrator("1) Preco by som ùa mal poc˙vaù?", 50); cout << "\n";FunctionDef.truenarrator("2) NeverÌm ti.",50); cout << "\n";FunctionDef.truenarrator("3) To je öialenstvo.",50); cout << "\n";FunctionDef.truenarrator("4) Dobre, mali by sme sa pohn˙ù.",50); 		
				cout << "\n\n\033[33m>>    ";
				diadec4 = FunctionDef.GVIclean(1,4);
				cout << "\033[0m";
				if(diadec4=="1"){
				...
				if(diadec4=="3"){
					FunctionDef.MCSC(mc,"To je öialenstvo.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(mc," Nerozumiem.", 80 ); FunctionDef.skip(250);FunctionDef.narratorclean(vc," Ale nem·m moc na v˝ber. Takûe ti vyhoviem.", 80 ); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("VysvetlÌm ti vöetko, co viem, akon·hle sa odtiaæto dostaneme.", 80); FunctionDef.skip(1500); relwvoid = 3;
				}
				if(diadec4=="4"){
					FunctionDef.MCSC(mc,"Dobre, mali by sme sa pohn˙ù.", 80); FunctionDef.skip(1000);
					cout << "\n";
				...
				cout << "\n";
				FunctionDef.narrator("Ch·pem, ûe to teraz asi nedok·ûeö pochopiù,", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," ale kaûdop·dne,", 100); FunctionDef.skip(250); 
				cout << "\n";
				FunctionDef.narrator("musÌme sa pohn˙ù. Takûe by som ti radil vstaù a odÌsù odtiaæto skÙr neû n·s niekto", 125); FunctionDef.skip(400);FunctionDef.narratorclean(vc," alebo horöie,",80); FunctionDef.skip(300);FunctionDef.narratorclean(vc," nieco,", 80 ); FunctionDef.skip(250);FunctionDef.narratorclean(vc," n·jde.", 80 );
				cout << "\n";
				FunctionDef.narrator("Medzit˝m, ti sk˙sim vysvetliù, co som myslel pod \"death\" a taktieû sk˙sime prÌsù na to, preco som v tvojej hlave", 80);
				cout << "\n";
				FunctionDef.narrator("a preco si nic nepam‰t·ö.", 80);
				cout << endl << endl;FunctionDef.truenarrator("1) Preco by som ùa mal poc˙vaù?", 50); cout << "\n";FunctionDef.truenarrator("2) NeverÌm ti.",50); cout << "\n";FunctionDef.truenarrator("3) Toto je nocn· mora, ûe?",50); cout << "\n";FunctionDef.truenarrator("4) Dobre, mali by sme sa pohn˙ù.",50); 		
				cout << "\n\n\033[33m>>    ";
				...
				if(diadec4=="2"){
					FunctionDef.MCSC(mc,"NeverÌm ti.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("NemusÌö mi veriù.", 75); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " StacÌ, keÔ pohneö s t˝m svojÌm zadkom skÙr, neû n·s nieco zabije.", 75); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.MCS("Fajn.", 100); FunctionDef.skip(1500); relwvoid = 2;
				}
				if(diadec4=="3"){
					FunctionDef.MCSC(mc,"Toto je nocn· mora, ûe?", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Ale no tak, vzchop sa.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," JasnÈ, ûe toto nie je nocn· mora.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," Aj keÔ moûno je, ale na tom nez·leûÌ.", 80); FunctionDef.skip(200); cout << "\n";
					FunctionDef.narrator("Umieraù v sne tieû stojÌ za dve veci.", 80);  FunctionDef.skip(400);FunctionDef.narratorclean(vc," Tak sa uû prosÌm ùa postav a spolu to nejako rozl˙skneme.", 80); FunctionDef.skip(1000); cout << "\n";
					FunctionDef.MCS("Dobre, vyhoviem ti.", 80); FunctionDef.skip(1500); relwvoid = 3;
				}
				if(diadec4=="4"){
					FunctionDef.MCSC(mc,"Dobre, mali by sme sa pohn˙ù.", 80); FunctionDef.skip(1000);
			...
			FunctionDef.MCSC(mc, "Ako je to moûnÈ?", 80); FunctionDef.skip(1000);
			cout << "\n";
			FunctionDef.narrator("NetuöÌm.", 80); FunctionDef.skip(500);FunctionDef.narratorclean(vc," Ale co viem,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc," je to,", 125);FunctionDef.narratorclean(vc," ûe by sme mali odtiaæto vypadn˙ù,", 100); FunctionDef.skip(500); cout << "\n";FunctionDef.narrator("nechceme, aby n·s tu cokoævek naölo, ûe?", 80); FunctionDef.skip(1000);FunctionDef.narratorclean(vc," Odpovede ti d·m po ceste,", 80); FunctionDef.skip(250); cout << "\n";FunctionDef.narrator("proste sa postav a daj do pohybu,", 80); FunctionDef.skip(200);FunctionDef.narratorclean(vc, " prosÌm.", 100); FunctionDef.skip(1000);
			cout << endl << endl;FunctionDef.truenarrator("1) Preco by som ùa mal poc˙vaù?", 50); cout << "\n";FunctionDef.truenarrator("2) NeverÌm ti.",50); cout << "\n";FunctionDef.truenarrator("3) Toto je nocn· mora, ûe?",50); cout << "\n";FunctionDef.truenarrator("4) Dobre, mali by sme sa pohn˙ù.",50); 		
			cout << "\n\n\033[33m>>    ";
			diadec3 = FunctionDef.GVIclean(1,4);
			cout << "\033[0m";
			...
			if(diadec3=="3"){
				...
				cout << "\n";
				FunctionDef.narrator("Ale no tak, vzchop sa.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," JasnÈ, ûe toto nie je nocn· mora.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," Aj keÔ, moûno je, ale na tom nez·leûÌ.", 80); FunctionDef.skip(200); cout << "\n";
				FunctionDef.narrator("Umieraù v sne tieû za moc nestojÌ.", 80);  FunctionDef.skip(400);FunctionDef.narratorclean(vc," Tak sa uû prosÌm ùa postav a spolu to nejako rozl˙skneme.", 80); FunctionDef.skip(1000); cout << "\n";
				FunctionDef.MCS("Dobre, vyhoviem ti.", 80); FunctionDef.skip(1500); relwvoid = 3;
			}
			if(diadec3=="4"){
				FunctionDef.MCSC(mc,"Dobre, mali by sme sa pohn˙ù.", 80); FunctionDef.skip(1000);
		...
		if(diadec2=="3"){
			FunctionDef.MCSC(mc, "Ale kde s˙ moje spomienky?", 80); FunctionDef.skip(400);FunctionDef.narratorclean(mc," Len tak si sem priöiel, a vöetky s˙ prec.", 80); FunctionDef.skip(400); FunctionDef.narratorclean(mc, " Co si snimi urobil?", 80); FunctionDef.skip(1000);
			cout << "\n";
			FunctionDef.narrator("Ja som tvojim spomienkam nic neurobil.", 80); FunctionDef.skip(500);FunctionDef.narratorclean(vc," Nic o tom neviem,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc," rovnako ako ty.", 125); FunctionDef.skip(400);FunctionDef.narratorclean(vc," myslÌö, ûe tu chcem byù?", 100); FunctionDef.skip(500); cout << "\n";FunctionDef.narrator("Umrel som ani neviem ako a zrazu som v cudzom tele,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " v tvojom tele,", 100);FunctionDef.narratorclean(vc, " ktorÈ nemÙûem kontrolovaù.", 100); FunctionDef.skip(250); cout << "\n";FunctionDef.narrator("Ale,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc," sme tam, kde sme,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc," tak pohni s t˝m svojÌm zadkom a postav sa.", 80); FunctionDef.skip(1000);
			cout << endl << endl;FunctionDef.truenarrator("1) Preco by som ùa mal poc˙vaù?", 50); cout << "\n";FunctionDef.truenarrator("2) NeverÌm ti.",50); cout << "\n";FunctionDef.truenarrator("3) Toto musÌ byù nocn· mora.",50); cout << "\n";FunctionDef.truenarrator("4) Dobre, mali by sme sa pohn˙ù.",50); 	
			cout << "\n\n\033[33m>>    ";
			diadec3 = FunctionDef.GVIclean(1,4);
			cout << "\033[0m";
			if(diadec3=="1"){
			...
			if(diadec3=="3"){
				FunctionDef.MCSC(mc,"Toto musÌ byù nocn· mora!...", 80);FunctionDef.narratorclean(mc, "ARGHHHH!", 80); FunctionDef.skip(1000);
				cout << "\n";
				FunctionDef.narrator("Nem·me na v˝ber,", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," musÌme spolupracovaù,", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc," jedine, ûe by si chcel umrieù.", 80); FunctionDef.skip(1000); 
				cout << endl << endl;FunctionDef.truenarrator("1) Dobre, tak poÔme.", 50); cout << "\n";FunctionDef.truenarrator("2) Moment, preco by som mal umrieù?",50); cout << "\n";FunctionDef.truenarrator("3) Nie! Chcem Ìsù vlastnou cestou.",50); cout << "\n";FunctionDef.truenarrator("4) Nepohnem sa odtiaæto, k˝m mi nepovieö, kto si.",50); 	
				cout << "\n\n\033[33m>>    ";
				diadec4 = FunctionDef.GVIclean(1,4); 
				cout << "\033[0m";
				if(diadec4=="1"){
					FunctionDef.MCSC(mc,"Dobre, tak poÔme.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("V˝borne.", 75);FunctionDef.narratorclean(vc," Vydajme sa t˝mto smerom.", 75); FunctionDef.skip(1500); relwvoid = 3;
				}
				if(diadec4=="2"){
					FunctionDef.MCSC(mc,"Pockaj, preco by som mal umrieù?", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Pretoûe budeö na tomto svete s·m,", 75); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " bez spomienok,", 75); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " ukradtnut˝ kaûdÈmu,", 75); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " a æahk˝ cieæ pre nepriateæov.", 75); FunctionDef.skip(250); cout << "\n";FunctionDef.narrator("V tomto lese nie sme sami.", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " A ja som ten, ktor˝ ti pomÙûe sa odtiaæto bezpecne dostaù.", 75); FunctionDef.skip(1000); 
					cout << "\n";
					FunctionDef.MCS("Preco sa mi toto deje?", 75); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Viem, ûe si vystraöen˝ a frustrovan˝.", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc," Ale ver mi,", 100); FunctionDef.skip(400);FunctionDef.narratorclean(vc," naöa jedin· öanca, ako preûiù, je postaviù sa pohn˙ù sa odtiaæto.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.MCS("Vyzer· to tak, ûe nem·m na v˝ber...", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc," Dobre,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(mc," budem spolupracovaù.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Super. Tak sa vydajme na cestu.", 80); FunctionDef.skip(1500); relwvoid = 3;
				}
				if(diadec4=="3"){
					FunctionDef.MCSC(mc,"Nie! Chcem Ìsù vlastnou cestou.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Vieö co, fajn, choÔ si svojou cestou, uvidÌme, kam n·s to zavedie.", 75); FunctionDef.skip(1500);
					// endingplaceholder1();
				}
				if(diadec4=="4"){
					FunctionDef.MCSC(mc,"Nepohnem sa odtiaæto, k˝m mi nepovieö, kto si.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Dobre, dobre.", 80);  FunctionDef.skip(500);FunctionDef.narratorclean(vc," Vöetko ti poviem...", 75); FunctionDef.skip(250);FunctionDef.narratorclean(vc," Vol·m sa Void.", 75); FunctionDef.skip(250);FunctionDef.narratorclean(vc," Bol som gener·lom frakcie", 75); cout << "\n";FunctionDef.narrator("ktorej meno bolo Terragnis.", 75); FunctionDef.skip(250);FunctionDef.narratorclean(vc," Boli sme vo vojne s inou frakciou, volali sa Slofridi.", 75); FunctionDef.skip(250);FunctionDef.narratorclean(vc," Bol som uprostred boja,", 80); FunctionDef.skip(250); cout << "\n";FunctionDef.narrator("keÔ ma zasiahol öÌp.", 75); FunctionDef.skip(250);FunctionDef.narratorclean(vc," Ale,", 100); FunctionDef.skip(400);FunctionDef.narratorclean(vc," nejak˝m z·zrakom,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc," som neumrel.", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc," Zrazu,", 100); FunctionDef.skip(250); cout << "\n";FunctionDef.narrator(" som videl ostrÈ svetlo, po ktorom nasledovala tma.", 75); FunctionDef.skip(250);FunctionDef.narratorclean(vc," KeÔ som priöiel k vedomiu, naöiel som sa v cudzom tele...", 75); FunctionDef.skip(250); cout << "\n";FunctionDef.narrator("v tvojom tele.", 75); FunctionDef.skip(400);FunctionDef.narratorclean(vc," A teraz,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc," sme tu.", 100); FunctionDef.skip(1500);
					cout << "\n";
					FunctionDef.MCS("Takûe si umriel a zobudil sa v mojom tele.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("¡no, v skratke.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.MCS("Ale ako je to moûnÈ?", 80); FunctionDef.skip(1000); 
					cout << "\n";
					FunctionDef.narrator("Povedal som ti vöetko, co viem.", 80);  FunctionDef.skip(500);FunctionDef.narratorclean(vc," Tieû by som r·d vedel viac,", 75); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " ale nic nezistÌme, pokiaæ tu budeme len tak sedieù.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.MCS("M·ö pravdu,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc," poÔme odtiaæto prec.", 75); FunctionDef.skip(1500);	relwvoid = 3;
					earlyexplanation = true;	
				}
			}
		}
		else if(diadec2=="4"){
			FunctionDef.MCSC(mc, "Klameö!", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " Schov·vaö sa za t˝mi stromami, ûe??", 80); FunctionDef.skip(1000);
			cout << "\n";
			FunctionDef.narrator("Nie som klam·r.", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " Iba vtedy, keÔ je to fakt nevyhnutnÈ.", 80); FunctionDef.skip(250); cout << "\n";FunctionDef.narrator("Ale zato ty by si sa mal pohn˙ù s t˝m zadkom, inak kvÙli tebe obidvaja umrieme.", 80); FunctionDef.skip(1000);
			cout << endl << endl;FunctionDef.truenarrator("1) Fajn, pohnime sa.", 50); cout << "\n";FunctionDef.truenarrator("2) Vyhoviem ti, keÔ mi pod·ö lepöie vysvetlenie.",50); cout << "\n";FunctionDef.truenarrator("3) Nech·pem, ako je to moûnÈ?",50); cout << "\n";FunctionDef.truenarrator("4) Tak toto ti neverÌm. Iba sa ma snaûÌö vyl·kaù von.",50);
			cout << "\n\n\033[33m>>    ";
			diadec3 = FunctionDef.GVIclean(1,4);
			cout << "\033[0m";
			if(diadec3=="1"){
				...
				cout << "\n";
				FunctionDef.narrator("Super.", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, "Vydajme sa tadiaæto,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " vyzer· to tam sæubne.", 80); FunctionDef.skip(1500); relwvoid = 3;
			}
			else if(diadec3=="2"){
				FunctionDef.MCSC(mc, "Vyhoviem ti, keÔ mi pod·ö lepöie vysvetlenie.", 80); FunctionDef.skip(1000);
				cout << "\n";
				FunctionDef.narrator("Povedal som ti,", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " fakt neviem.", 80);FunctionDef.narratorclean(vc, " JedinÈ co viem je, ûe som uv‰znen˝ v tvojej hlave a ûe n·s dostaneö ", 80); cout << "\n";FunctionDef.narrator("do hrobu pokiaæ sa odtiaæto nepohneö,",80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " tak ùa prosÌm,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " pohni sa,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " a poviem ti viac po ceste.", 80); FunctionDef.skip(1000); 
				cout << endl << endl;FunctionDef.truenarrator("1) Preco by som ùa mal poc˙vaù?", 50); cout << "\n";FunctionDef.truenarrator("2) Nie, st·le ti neverÌm. Odmietam tomuto vöetkÈmu veriù.", 50); cout << "\n";FunctionDef.truenarrator("3) Fajn, tak sa teda d·m do pohybu", 50);
				cout << "\n\n\033[33m>>    ";
				diadec4 = FunctionDef.GVIclean(1,3);
				cout << "\033[0m";
				if(diadec4=="1"){
					FunctionDef.MCSC(mc, "Preco by som ùa mal poc˙vaù?", 80); FunctionDef.skip(1000);
					FunctionDef.narrator("Nem·ö na v˝ber.", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " Aj keÔ, moûno m·ö,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " rob si co chceö,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " chceö tak veæmi umrieù?", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " ProsÌm, v kæude si choÔ.", 80); FunctionDef.skip(1000);
					cout << endl << endl;FunctionDef.truenarrator("1) PÙjdem svojou vlastnou cestou.", 50); cout << "\n";FunctionDef.truenarrator("2) Dobre, pockaj, ospravedlÚujem sa. UrobÌm, co chceö.", 50);
					cout << "\n\n\033[33m>>    ";
					diadec5 = FunctionDef.GVIclean(1,4);
					cout << "\033[0m";
					if(diadec5=="1"){
						FunctionDef.MCSC(mc, "PÙjdem svojou vlastnou cestou.", 50); FunctionDef.skip(1000);
						cout << "\n";
						FunctionDef.narrator("Vieö co, fajn, choÔ kam chceö, uvidÌme, kam sa dostaneö.", 80);
						// endingplaceholder1();
					}
					else{
						FunctionDef.MCSC(mc, "Dobre,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " prep·c.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(mc, " UrobÌm, co chceö.", 80); FunctionDef.skip(1000);
						cout << "\n";
						FunctionDef.narrator("Tak predsa len.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Nuû,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " nem·me casu nazvyö,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " povstaÚ a vydaj sa tamtou cestou.", 80); FunctionDef.skip(1500); relwvoid = 3;
					}
				}
				else if(diadec4=="2"){
					FunctionDef.MCSC(mc, "Nie,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " st·le ti neverÌm.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(mc, " Odmietam tomuto vöetkÈmu veriù.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Aha,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " takûe tu chceö umrieù?", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " ProsÌm,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " choÔ kam chceö.", 80); FunctionDef.skip(1000);
					cout << endl << endl;FunctionDef.truenarrator("1) PÙjdem tam, kde chcem.", 50); cout << "\n";FunctionDef.truenarrator("2) Vieö co, fajn, poc˙vnem ùa.", 50);
					cout << "\n\n\033[33m>>    ";
					diadec5 = FunctionDef.GVIclean(1,2);
					cout << "\033[0m";
					if(diadec5=="1"){
						FunctionDef.MCSC(mc, "PÙjdem tam, kde chcem.", 50); FunctionDef.skip(1000);
						cout << "\n";
						FunctionDef.narrator("Vieö co, ako chceö, choÔ svojou cestou. AspoÚ uvidÌme, kam to vedie.", 80);
						// endingplaceholder1();
					}
					else{
						FunctionDef.MCSC(mc, "Vieö co,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " fajn,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " poc˙vnem ùa.", 80); FunctionDef.skip(1000);
						cout << "\n";
						FunctionDef.narrator("Konecne...", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " MÙûeme sa pohn˙ù.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Postav sa,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " a poÔme touto cestou.", 80); FunctionDef.skip(1500); relwvoid = 1;
					}
				}
				else{
					FunctionDef.MCSC(mc, "Fajn,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " d·m sa do pohybu.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Dobre,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " v poriadku,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " mali by sme Ìsù tadiaæto.", 80); FunctionDef.skip(1500); relwvoid = 3;
				}
			}
			else if(diadec3=="3"){
				FunctionDef.MCSC(mc, "Nerozumiem, ako je to moûnÈ?", 80); FunctionDef.skip(1000);
				cout << "\n";
				FunctionDef.narrator("Tieû tomu nerozumiem.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " MÙûeme to spolu zistiù,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " len sa uû,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " prosÌm ùa,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " pohni.", 100); FunctionDef.skip(1000); 
				cout << endl << endl;FunctionDef.truenarrator("1) JasnÈ, prep·c, d·m sa do pohybu.", 50); cout << "\n";FunctionDef.truenarrator("2) Co ak nie?", 50);
				cout << "\n\n\033[33m>>    ";
				diadec4 = FunctionDef.GVIclean(1,2);
				cout << "\033[0m";
				if(diadec4=="1"){
					FunctionDef.MCSC(mc, "JasnÈ,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " prep·c,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " d·m sa do pohybu.", 90); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("V pohode", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " poÔme tadiaæto.", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " Zd· sa mi, ûe tam nieco vidÌm.", 80); FunctionDef.skip(1000); relwvoid = 3;
				}
				else{
					FunctionDef.MCSC(mc, "Co ak,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " nie?", 100); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Fakt si chceö zo mÚa urobiù nepriateæa?", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Pam‰taj,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " som v tvojej hlave,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " a kedykoævek mÙûem pre teba z tohto urobiù ", 80); cout << "\n";FunctionDef.narrator("peklo na Zemi.", 80); FunctionDef.skip(1000);
					cout << endl << endl;FunctionDef.truenarrator("1) ¡no. A chcem Ìsù svojou cestou.", 50); cout << "\n";FunctionDef.truenarrator("2) Zmenil som n·zor, prep·c, uû ùa budem poc˙vaù.", 50); 
					cout << "\n\n\033[33m>>    ";
					diadec5 = FunctionDef.GVIclean(1,2);
					cout << "\033[0m";
					if(diadec5=="1"){
						FunctionDef.MCSC(mc, "¡no. A chcem Ìsù svojou cestou.", 80); FunctionDef.skip(1000);
						cout << "\n";
						FunctionDef.narrator("Vieö co, rob si ako chceö, uvidÌme, kam sa dostaneme.", 80); FunctionDef.skip(1500);
						// endingplaceholder1();
					}
					else{
						FunctionDef.MCSC(mc, "Zmenil som n·zor,",80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " prep·c,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " uû ùa budem poc˙vaù.", 80); FunctionDef.skip(1000);
						cout << "\n";
						FunctionDef.narrator("Tak nakoniec predsa len,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " fajn,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc, "poÔme tadiaæto,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " Sk˙sim ti vysvetliù vöetko, co viem.", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " Pokiaæ chceme spolu naûÌvaù", 80); cout << "\n";FunctionDef.narrator("v mieri,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " tak musÌme spolupracovaù.", 80); FunctionDef.skip(1500); relwvoid = 2;
					}
				}
			}
			else{
				FunctionDef.MCSC(mc, "Na to ti neskocÌm.", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " SnaûÌö sa ma dostaù von.", 80); FunctionDef.skip(1000);
				cout << "\n";
				FunctionDef.narrator("V·ûne?", 75); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " Poc˙vaj,", 80);FunctionDef.narratorclean(vc," nem·ö na v˝ber,", 80);FunctionDef.narratorclean(vc," pohni sa,",100);FunctionDef.narratorclean(vc," nem·ö co stratiù.", 80); FunctionDef.skip(1000); 
				cout << endl << endl;FunctionDef.truenarrator("1) PrestaÚ na mÚa hovoriù, chcem Ìsù vlastnou cestou.", 50); cout << "\n";FunctionDef.truenarrator("2) Fajn.", 50); cout << "\n";FunctionDef.truenarrator("3) Prep·c, chcel som sa iba uistiù. MÙûeme Ìsù.", 50); 
				cout << "\n\n\033[33m>>    ";
				diadec4 = FunctionDef.GVIclean(1,3);
				cout << "\033[0m";
				if(diadec4=="1"){
					FunctionDef.MCSC(mc, "PrestaÚ na mÚa hovoriù,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " chcem Ìsù vlastnou cestou.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Vieö co, fajn, choÔ, kadiaæ chceö, uvidÌme, co z tohto vznikne.", 80); FunctionDef.skip(1500);
					// endingplaceholder1();
				}
				else if(diadec4=="2"){
					FunctionDef.MCSC(mc, "Fajn.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("Tvoj postoj je otrasn˝,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " mohol by si prejaviù aspoÚ trochu ˙cty tomu hlasu, co m·ö v hlave.", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, "Poobzeraj sa trochu,", 80); FunctionDef.skip(250); cout << "\n";FunctionDef.narrator("vyzer· to, ûe tamt˝m smerom nieco je.", 80); FunctionDef.skip(1500); relwvoid = 1;
				}
				else{
					FunctionDef.MCSC(mc, "Prep·c,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " chcel som sa len uistiù.", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " MÙûeme sa daù do pohybu.", 80); FunctionDef.skip(1000);
					cout << "\n";
					FunctionDef.narrator("To je v pohode,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " Ch·pem ùa.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " tak sa teda vydajme na cestu.", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " Vyzer·, ûe t·to cesta niekam vedie.", 80); FunctionDef.skip(1500); relwvoid = 3;
				}
			}
		}
		cout << "\n";
		...
		while(true){
			...
			cout << "\033[0m";
			if(decision1=="1"){
				FunctionDef.MCSC(mc, "Chcem sa pozrieù na tie stromy.", 80);
				cout << endl << endl;
				d1n(lang);
				FunctionDef.skip(500);
				cout << endl << endl;
				...
				FunctionDef.MCSC(mc, "MusÌm vidieù tie stromy.", 80);
				cout << endl << endl;
				d1n(lang);
				FunctionDef.skip(500);
				cout << endl << endl;
				...
				FunctionDef.MCSC(mc, "Chcel by som vedieù, kam vedie t·to cesta.", 80);
				cout << endl << endl;
				FunctionDef.truenarrator("Okej, rozhodol si sa zistiù, ak˝ je cieæ tej cesty. Si si ist˝, ûe nechceö naskÙr skontrolovaù\n      to opustenÈ miesto medzi stromami, kde mÙûu leûaù uûitocnÈ veci?", 50);
				cout << endl << endl;
				FunctionDef.skip(250);
				FunctionDef.truenarrator("1) Pokracovaù v pÙvodnom rozhodnutÌ a Ìsù presk˙maù cestu\n      2) Upustiù od pÙvodnÈho rozhodnutia a Ìsù presk˙maù opustenÈ miesto medzi stromami", 50);
				cout << endl << endl;
				FunctionDef.flushInput();
				cout << "\033[33m>>    ";
				decision1sub1 = FunctionDef.GVI(1,2);
				cout << "\033[0m";
				if(decision1sub1=="1"){
					FunctionDef.MCSC(mc, "Veæmi chcem vedieù, kam to vedie.", 80);
					cout << endl << endl;
					FunctionDef.truenarrator("Nuû dobre, rozhodol si sa vydaù na dobrodr˙ûnu cest- hups, tak nic, umrel si. DÙvod: NEZN¡MY", 50);
					FunctionDef.skip(400);
					lives--; deathsequence(lang);
				}
				else if(decision1sub1=="2"){
					FunctionDef.MCSC(mc, "MyslÌm, ûe by sme mali najskÙr skontrolovaù to opustenÈ miesto.", 80);
					cout << endl << endl;
					FunctionDef.truenarrator("Nakoniec si sa rozhodol skontrolovaù to divnÈ miesto s opusten˝mi vecami na zemi. A pred sebou m·ö:",50); cout << "\n\n";FunctionDef.truenarrator("- Caj",50); cout << "\n";FunctionDef.truenarrator("- éelezn˝ mec",50); cout << "\n";FunctionDef.truenarrator("- æahko-zniciteænÈ zlatÈ brnenie",50); cout << "\n";FunctionDef.truenarrator("- ätÌt",50); cout << "\n";FunctionDef.truenarrator("- ElixÌr l·sky",50); cout << "\n";FunctionDef.truenarrator("- Palicu",50); cout << "\n";FunctionDef.truenarrator("- Obv‰zy x3",50); cout << "\n";FunctionDef.truenarrator("- Mal˝ batoh", 50);
					break;
				}
			}
			else if(decision1=="5"){
				FunctionDef.MCSC(mc, "R·d by som najskÙr skontroloval to opustenÈ miesto medzi stromami.", 80);
				cout << endl << endl;
				FunctionDef.truenarrator("Skvel· voæba, teda minim·lne lepöia ako \"stromy\", priblÌûil si sa k opusten˝m veciam a vidÌö nasledovnÈ:",50); cout << "\n\n";FunctionDef.truenarrator("- Caj",50); cout << "\n";FunctionDef.truenarrator("- éelezn˝ mec",50); cout << "\n";FunctionDef.truenarrator("- ºahko-zniciteænÈ zlatÈ brnenie",50); cout << "\n";FunctionDef.truenarrator("- ätÌt",50); cout << "\n";FunctionDef.truenarrator("- ElixÌr l·sky",50); cout << "\n";FunctionDef.truenarrator("- Palicu",50); cout << "\n";FunctionDef.truenarrator("- Obv‰zy x3",50); cout << "\n";FunctionDef.truenarrator("- Mal˝ batoh", 50); break;
			}
		}
		...
		if(b0=="1"){
			FunctionDef.narrator("Nielen, ûe si ˙pln˝ kretÈn,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc," ale eöte aj ˙plne hl˙py.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Preco by si si vybral stromy 3-kr·t po sebe?", 80); FunctionDef.skip(400); cout << "\n";FunctionDef.narrator("Vieö co,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " Radöej to ani nechcem pocuù,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " prestaÚ str·caù n·ö cas, zober veci and vypadni odtiaæto.", 80); FunctionDef.skip(1000);
			cout << endl << endl;FunctionDef.truenarrator("1) Prep·c, fakt som chcel tie stromy.", 50); cout << "\n";FunctionDef.truenarrator("2) Ja som kretÈn?! Ty si kretÈn!", 50); cout << endl << endl; 
			cout << "\033[33m>>    ";
			diadec1 = FunctionDef.GVIclean(1,2);
			cout << "\033[0m";
			if(diadec1=="1"){
				...
				cout << "\n";
				FunctionDef.narrator("Uloû to,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " zober veci a CHOœ!", 80); FunctionDef.skip(1000); 
				cout << endl << endl;FunctionDef.truenarrator("Zobral si veci a odteraz ho mÙûeö n·jsù vo svojom invent·ri, ktor˝m je batoh.", 50); FunctionDef.skip(500); cout << "\n";
			}
			else{
				FunctionDef.MCSC(mc, "Ja som kretÈn?!", 75); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " Ty si kretÈn!", 80); FunctionDef.skip(400);FunctionDef.narratorclean(mc, " Ty si ten, ktor˝m sa mi vtrel do hlavy.", 80); FunctionDef.skip(1000);
				cout << "\n";
				FunctionDef.narrator("Ja?!", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Vieö co,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " choÔ do certa,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " povedal som ti, co sa stalo a preco som tu.", 80); FunctionDef.skip(400); cout << "\n";FunctionDef.narrator("Tak l·skavo prestaÚ, zober veci a padaj prec odtiaæto skÙr, neû umrieme.", 80); FunctionDef.skip(1000);
				cout << endl << endl;FunctionDef.truenarrator("Zobral si veci a odteraz ho mÙûeö n·jsù vo svojom invent·ri, ktor˝m je batoh.", 50); FunctionDef.skip(500); cout << endl << endl;
			}
			FunctionDef.narrator("Fakt sa snaûÌm byù mil˝,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " ale ty ma proste nenech·ö.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Co navrhujeö teraz?", 80); FunctionDef.skip(400); cout << "\n";FunctionDef.narrator("PÙjdeme touto cestou?", 80); FunctionDef.skip(1000);
			cout << endl << endl;FunctionDef.truenarrator("1) Õsù cestou, ktor˙ navrhol Void.", 80); cout << "\n";FunctionDef.truenarrator("2) Õsù opacn˝m smerom.", 50); cout << endl << endl;
			cout << "\033[33m>>    ";
			b1pathdec = FunctionDef.GVIclean(1,2);
			cout << "\033[0m";
			if(b1pathdec=="1"){
				FunctionDef.MCSC(mc, "Fajn, poÔme teda tvojou cestou.", 80); FunctionDef.skip(1000);
				cout << "\n";
				FunctionDef.narrator("AspoÚ ûe poc˙vaö.", 80); FunctionDef.skip(1000); relwvoid = 2;
			}
			else{
				FunctionDef.MCSC(mc, "Vieö co?", 80); FunctionDef.skip(400);FunctionDef.narratorclean(mc, "nechce sa mi ùa poc˙vaù.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(mc, "Radöej by som öiel tdiaæto,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " nem·me co stratiù.", 80); 
				cout << "\n";
				FunctionDef.narrator("Vieö co,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " fajn,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " choÔ do ktorejkoævek riti chceö.", 80); FunctionDef.skip(1000); relwvoid = 1; d="7";
			}
		}
		else if(b0=="2"){
			FunctionDef.narrator("Preco by si si vybral stromy 3-kr·t po sebe?", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Nie si n·hodou", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, "-", 200);FunctionDef.narratorclean(vc, " OspravedlÚujem sa.", 100); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Tvoj v˝ber ma frustroval.", 80); FunctionDef.skip(400); cout << "\n";FunctionDef.narrator("Je ˙plne jasnÈ, ûe tam nic nie je,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " str·came n·ö cas.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Mali by sme Ìsù Ôalej,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " zobver veci a poÔ.", 80); FunctionDef.skip(1000);
			cout << "\n"; 
			FunctionDef.MCS("Prep·c.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(mc, " PoÔme t˙ cestu, co sme videli predt˝m.", 80); FunctionDef.skip(1000);
			cout << endl << endl;FunctionDef.truenarrator("Zobral si veci a odteraz ho mÙûeö n·jsù vo svojom invent·ri, ktor˝m je batoh.", 50); FunctionDef.skip(500);
			cout << endl << endl; 
			FunctionDef.narrator("Pravda,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " t· cesta by n·s mohla zaviesù niekde, kde mÙûeme n·jsù odpovede.", 80); FunctionDef.skip(1000);
		}
		else if(b0=="3"){
			FunctionDef.narrator("Zober to a poÔme.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " A inak,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " co je to s tebou?", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Upusti od tak˝chto rozhodnutÌ,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " bude lepöie, keÔ sa dohodneme.", 80); FunctionDef.skip(1000);
			cout << endl << endl;FunctionDef.truenarrator("Zobral si veci a odteraz ho mÙûeö n·jsù vo svojom invent·ri, ktor˝m je batoh.", 50); FunctionDef.skip(500);
			cout << endl << endl;FunctionDef.truenarrator("1) OspravedlÚujem sa za to...", 50); cout << "\n";FunctionDef.truenarrator("2) ...", 50); cout << endl << endl;
			cout << "\033[33m>>    ";
			diadec1 = FunctionDef.GVIclean(1,2);
			cout << "\033[0m";			 
			if(diadec1=="1"){
				FunctionDef.MCSC(mc, "OspravedlÚujem sa za to...", 80); FunctionDef.skip(400);FunctionDef.narratorclean(mc, " Nemyslel som to tak,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " bol som iba opatrn˝.", 80); FunctionDef.skip(400); cout << "\n";FunctionDef.MCS("But Ale aspoÚ uû vidÌm, ûe nie si zl˝ a ûe si naozaj v mojej hlave.", 80); FunctionDef.skip(1000);
				cout << "\n";
				FunctionDef.narrator("V poriadku,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " rozumiem.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " CenÌm si tvoje ospravedlnenie.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Mali by sme sa daù do pohybu,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " poÔme tou cestou, ktor˙ sme videli predt˝m.", 80); FunctionDef.skip(1000);
				relwvoid = 3;
			}
			else{
				FunctionDef.MCSC(mc, "...", 80); FunctionDef.skip(1000);
				cout << "\n";
				FunctionDef.narrator("Ja-", 150); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " ch·pem, ûe to musÌ byù z tvojho pohæadu ùaûkÈ.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " KeÔ sa na to pozer·m z tvojej perspektÌvy,", 80);FunctionDef.narratorclean(vc, " tieû by som sebe neveril.", 80); FunctionDef.skip(400); cout << "\n";FunctionDef.narrator("Ch·pem ùa,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " prep·c.", 100); FunctionDef.skip(1000);
				cout << "\n";
				FunctionDef.MCS("œakujem,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " ale ja som ten, ktor˝ by sa tu mal ospravedlÚovaù.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(mc, " Ale m·ö pravdu,", 80);FunctionDef.narratorclean(mc, " mali by sme sa daù do pohybu a o tomto sa porozpr·vame neskÙr.", 80); FunctionDef.skip(1000); 
				relwvoid = 3;
			}
		}
		else if(b0=="4"){
			FunctionDef.narrator("V˝borne.", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc," Tie veci mÙûu byù pre n·s uûitocnÈ.", 80); FunctionDef.skip(400); FunctionDef.narratorclean(vc, " Zober ich a poÔme Ôalej.", 80); FunctionDef.skip(1000);
			cout << endl << endl;FunctionDef.truenarrator("Zobral si veci a odteraz ho mÙûeö n·jsù vo svojom invent·ri, ktor˝m je batoh.", 50); FunctionDef.skip(500);
			cout << endl << endl;
			FunctionDef.MCS("Dobr˝ n·pad,", 80); FunctionDef.skip(250); FunctionDef.narratorclean(mc, " nasledujme t˙ cestu, ktor˙ sme videli predt˝m,", 80); FunctionDef.skip(250); FunctionDef.narratorclean(mc, " h·dam, ûe vedie do mesta.",80); FunctionDef.skip(400); FunctionDef.narratorclean("\033[33m", " NemÙûe predsa viesù odnikiaæ nikam,", 80);FunctionDef.narratorclean("\033[33m", " ûe?", 100); FunctionDef.skip(1000);
		}
		else if(b0=="5"){
			FunctionDef.narrator("UûasnÈ,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " mali sme riadne öùastie.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Mali by sme pokracovaù v pohybe a dostaù sa odtiaæto,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " hneÔ ako zoberieö tie veci.", 80); FunctionDef.skip(1000);
			cout << endl << endl;FunctionDef.truenarrator("Zobral si veci a odteraz ho mÙûeö n·jsù vo svojom invent·ri, ktor˝m je batoh.", 50); FunctionDef.skip(500);
			cout << endl << endl;FunctionDef.MCS("S˙hlasÌm,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " mali by sme sa odtiaæto spakovaù,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " m·ö pravdu,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " asi by sme mohli Ìsù tou cestou, ktor˙ sme videli predt˝m,", 80); FunctionDef.skip(250); cout << "\n";FunctionDef.MCS("isto niekam vedie.", 80); FunctionDef.skip(1000);			
			cout << "\n";
			FunctionDef.narrator("Presne,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " niekam vedie,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " musÌ,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " neboj sa.", 80); FunctionDef.skip(1000);
		}
		else if(b0=="6"){
			FunctionDef.narrator("MÙûeö sa prestaù tak extrÈmne zaujÌmaù o stromy, zobraù tie veci a vypadn˙ù odtiaæto,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " trkvas?", 80); FunctionDef.skip(1000);
			cout << "\n";
			...
			FunctionDef.narrator("Vypadneme odtiaæto,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " tamtou cestou.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " ChoÔ a nic nehovor.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Tvoja rec ma zacÌna iritovaù", 80); cout << "\n";FunctionDef.narrator("a to sme sa spoznali len 3 min˙ty dozadu.", 80); FunctionDef.skip(1000);			
		}
		else if(b0=="7"){
			FunctionDef.narrator("Super,", 100); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " niekam sa dost·vame.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Ale preco si si 2-kr·t vybral stromy?", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " PrÌde mi to dosù m‰t˙ce,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " ale co uû,", 80); FunctionDef.skip(250); cout << "\n";FunctionDef.narrator("mali by sme pojracovaù Ôalej,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " mÙûeme Ìsù tou cestou, ktor˙ sme videli predt˝m.", 80); FunctionDef.skip(1000);
			cout << "\n";
			cout << endl << endl;FunctionDef.truenarrator("Zobral si veci a odteraz ho mÙûeö n·jsù vo svojom invent·ri, ktor˝m je batoh.", 50); FunctionDef.skip(500);
			cout << "\n";
			FunctionDef.MCS("M·m ich.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(mc, " Vydajme sa teda tamtou cestou.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(mc, " Moûno niekam vedie.", 80); FunctionDef.skip(1000);					
			cout << "\n";
			FunctionDef.narrator("Mala by niekam viesù,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " predpoklad·m, ûe do dediny.", 80); FunctionDef.skip(1000);		
		}	
		else if(b0=="8"){
			FunctionDef.narrator("V˝borne,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " zober ich,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " mÙûu byù uûitocnÈ.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " A hneÔ potom,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " by sme sa mali daù do pohybu,", 80); FunctionDef.skip(250); cout << "\n";FunctionDef.narrator("mÙûeme Ìsù tou cestou, ktor˙ sme videli predt˝m.", 80); FunctionDef.skip(1000);
			cout << "\n";
			FunctionDef.MCS("Dobre.", 100); FunctionDef.skip(1000);
			cout << endl << endl;FunctionDef.truenarrator("Zobral si veci a odteraz ho mÙûeö n·jsù vo svojom invent·ri, ktor˝m je batoh.", 50); FunctionDef.skip(500);
			cout << "\n";
			FunctionDef.narrator("Super,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " mÙûeö sa daù do pohybu,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " sk˙s byù nen·padn˝,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " nikdy nevieme, kto tu s nami mÙûe byù.", 80); FunctionDef.skip(1000);			
		}
		else if(b0=="9"){
			FunctionDef.narrator("Preco by si iöiel 2-kr·t k stromom?", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Absol˙tna strata casu.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " Proste iba zober tie veci a daj sa do pohybu.", 80); FunctionDef.skip(1000);
			cout << "\n";
			FunctionDef.MCS("OspravedlÚujem sa za to,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " ale s˙hlasÌm.", 100); FunctionDef.skip(250);FunctionDef.narratorclean(mc, " Pohnem sa, hneÔ ako to zoberiem.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(mc, " Mali by sme Ìsù tou cestou, ktor˙ sme videli predt˝m.", 80); FunctionDef.skip(1000);
			cout << endl << endl;FunctionDef.truenarrator("Zobral si veci a odteraz ho mÙûeö n·jsù vo svojom invent·ri, ktor˝m je batoh.", 50); FunctionDef.skip(500);
			cout << "\n";FunctionDef.narrator("MyslÌm, ûe je to naöa jedin· moûnosù.", 80); FunctionDef.skip(400);FunctionDef.narratorclean(vc, " MusÌ viesù do dediny,", 80); FunctionDef.skip(250);FunctionDef.narratorclean(vc, " na sto percent.", 80); FunctionDef.skip(1000);			 
		}
		if(b0!="1"&&relwvoid!=1){
			cout << endl << endl;FunctionDef.truenarrator("Si obkæ˙cen˝ svorkou vlkov, ktor· je prepraven· na teba za˙tociù. MusÌö sa br·niù.", 50); cout << "\n";FunctionDef.truenarrator("MusÌö sa r˝chlo rozhodn˙ù, co urobÌö: ", 50); cout << endl << endl;FunctionDef.truenarrator("1) POuûiù palicu a bojovaù s nimi",50); FunctionDef.skip(200); cout << "\n";FunctionDef.truenarrator("2) Pouûiù mec a bojovaù s nimi", 50); cout << "\n";FunctionDef.truenarrator("3) Sk˙siù ich odl·kaù palicou namiesto bojovania.", 50); cout << endl << endl;
			cout << "\033[33m>>    ";
			decision2 = FunctionDef.GVI(1,3);
			cout << "\033[0m" << endl;
			if(decision2=="1"){
				FunctionDef.truenarrator("Rozhodol si sa zobraù palicu a s Úou sa vrhn˙ù do boja. Zah·jujem s˙boj.", 50); FunctionDef.skip(200);
				ambush("Wolves", lang); 
			}
			else if(decision2=="2"){
				FunctionDef.truenarrator("Rozhodol si sa zobraù mec a s nÌm sa vrhn˙ù do boja. Zah·jujem s˙boj.", 50); FunctionDef.skip(200);
				ambush("Wolves", lang); 
			}
			else if(decision2=="3"){
				int wolfescapechance = rand()%4;
				if(wolfescapechance==1){
					FunctionDef.truenarrator("Sk˙sil si vlkov odl·kaù palicou. Hodil  si ho a podarilo sa ti ich odl·kaù.", 50); // Achievement, 25% chance to work 
				}
				else{
					FunctionDef.truenarrator("Sk˙sil si vlkov odl·kaù palicou, ale vÙbec nezareagovali a roztrhali ùa ka k˙sky.", 50); FunctionDef.skip(400); cout << "\n";
					lives--; deathsequence(lang);
				}
			}
		}
		...
		if(c=="1"){
			FunctionDef.narrator("Zbl·znil si sa?", 80); FunctionDef.skip(400); FunctionDef.narratorclean(vc, " Mohol si n·s zabiù.", 80); FunctionDef.skip(400); FunctionDef.narratorclean(vc, " M·ö öùastie, ûe to vyölo.", 80); FunctionDef.skip(400); cout << "\n"; FunctionDef.narrator("No nic, pokracujme Ôalej po ceste.", 80); FunctionDef.skip(1000);
			cout << "\n"; 
			FunctionDef.MCS("Bol to dobr˝ n·pad,", 80); FunctionDef.skip(250); FunctionDef.narratorclean(mc, " v boji by sme mohli umrieù.", 80); FunctionDef.skip(400); FunctionDef.narratorclean(mc, " Odl·kal som ich a teraz sme v bezpecÌ.", 80); FunctionDef.skip(400); cout << "\n"; FunctionDef.MCS("Idem teda Ôalej.", 80); FunctionDef.skip(1000);
		}
		else if(c=="2"){
			FunctionDef.narrator("Wau!", 80); FunctionDef.skip(400); FunctionDef.narratorclean(vc, " To s tou palicou bol dobr˝ n·pad,", 80); FunctionDef.skip(250); FunctionDef.narratorclean(vc, " necakal som, ûe to bude fungovaù.", 80); FunctionDef.skip(1000);
			cout << "\n";
			FunctionDef.MCS("Tieû som si nemyslel, ûe to mÙûe fungovaù,", 80); FunctionDef.skip(250); FunctionDef.narratorclean(mc, " ale som r·d, ûe to vyölo.", 80); FunctionDef.skip(400); FunctionDef.narratorclean(mc, " Nezastavujme a poÔme Ôalej.", 80); FunctionDef.skip(1000);
		}
		else if(c=="3"){
			FunctionDef.narrator("Preco by si pouûÌval na vlkov palicu?", 80); FunctionDef.skip(400); FunctionDef.narratorclean(mc, " M·me öùastie, ûe sme st·le naûive.", 80); FunctionDef.skip(1000);
			cout << "\n"; 
			FunctionDef.MCS("Bolo to veæmi spont·nne rozhodnutie.", 80); FunctionDef.skip(400); FunctionDef.narratorclean(mc, " Moûno to bolo ub·hlenÈ,", 80); FunctionDef.skip(250); FunctionDef.narratorclean(mc, " ale kaûdop·dne sme vyhrali.", 80); FunctionDef.skip(400); FunctionDef.narratorclean(mc, " Pokracujme Ôalej.", 80); FunctionDef.skip(1000); 
		}
		else if(c=="4"){
			FunctionDef.narrator("ZaujÌmavÈ...?", 100); FunctionDef.skip(400); FunctionDef.narratorclean(vc, " Dobr· pr·ca,", 80); FunctionDef.skip(250); FunctionDef.narratorclean(vc, " ocividne vieö, ako pouûÌvaù palicu.", 80); FunctionDef.skip(400); FunctionDef.narratorclean(vc, " Ale preco si namiesto toho nepouûil mec?", 80); FunctionDef.skip(1000);
			cout << "\n";
			FunctionDef.MCS("NetuöÌm.", 80); FunctionDef.skip(400); FunctionDef.narratorclean(mc, " Zobral som prv˙ vec, co mi priöla do rany a n·hodou to bola zrovna palica,", 80); FunctionDef.skip(250); FunctionDef.narratorclean(mc, " tak som ju pouûil.", 80); FunctionDef.skip(400); FunctionDef.narratorclean(mc, " Ale na to nez·leûÌ.", 80); FunctionDef.skip(400); FunctionDef.narratorclean(mc, " Vyhrali sme,", 100); FunctionDef.skip(250); FunctionDef.narratorclean(mc, " tak sa dajme to pohybu.", 80); FunctionDef.skip(1000);
		}
		else if(c=="5"){
			FunctionDef.narrator("AspoÚ ûe vieö bojovaù s mecom.", 80); FunctionDef.skip(400); FunctionDef.narratorclean(vc, " Pokracuj smerom vpred.", 80); FunctionDef.skip(1000);
			cout << "\n";
			FunctionDef.MCS("œakujem za uznanie, asi...", 80); FunctionDef.skip(400); FunctionDef.narratorclean(mc, " Zatiaæ si mi nebol veæmi n·pomocn˝,", 80); FunctionDef.skip(250); FunctionDef.narratorclean(mc, " vieö o tom?", 80); FunctionDef.skip(400); cout << "\n"; FunctionDef.MCS("Ale to asi nie je tvoja chyba.", 80); FunctionDef.skip(400); FunctionDef.narratorclean(mc, " Pokracujme Ôalej.", 80); FunctionDef.skip(400);
		}
		else if(c=="6"){
			FunctionDef.narrator("⁄ûasnÈ,", 80); FunctionDef.skip(250); FunctionDef.narratorclean(vc, " bol si super!", 80); FunctionDef.skip(400); FunctionDef.narratorclean(vc, " Mali by sme sa pohn˙ù Ôalej skÙr, neû sem prÌde Ôalöia svorka.", 80); FunctionDef.skip(400); cout << "\n"; FunctionDef.narrator("Som si ist˝, ûe je ich tu veæa.", 80); FunctionDef.skip(1000);
			cout << "\n";
			FunctionDef.MCS("œakujem,", 80); FunctionDef.skip(250); FunctionDef.narratorclean(mc, " a s˙hlasÌm.", 80); FunctionDef.skip(400); FunctionDef.narratorclean(mc, " PoÔme teda Ôalej.", 80); FunctionDef.skip(1000);		
		}
		sequence2(language);
		}
	}
